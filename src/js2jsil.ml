...
let process_file path =
  ...
  let e_str = IO_Utils.load_js_file path in
    let e_str = if (!symb_testing) then JS_PreParser.stringify_cosette e_str else e_str in
  ...
  JS2JSIL_Constants.prefix_anonymous_func_name := "_" ^ base_name ^ "_";
  let (ext_prog : EProg.t), _, _ = js2jsil e offset_converter (!verification) base_name in
