  ...
  let loc, prop = 
    match loc, prop with 
      | Loc loc, String prop -> 
            loc, prop 
      | _ -> 
            raise (Failure (Printf.sprintf "Illegal get_cell Loc: %s, Prop: %s" (str loc) (str prop))) in 
...
let assume_a (state : t) (ps : Formula.t list) : t option = 
  ...
  if List.for_all (fun expr_b -> expr_b = Some (Bool true)) bs 
    then Some state 
    else None  
...
let str ?(which_store=None) ?(which_heap=None) state = 
  ...
  let which_heap = 
    ...
    | None -> which_heap
    | Some vars ->  
        let vars = CCommon.SS.filter (fun x -> CStore.M.mem store x) vars in 
        let locs : SS.t = SS.fold (fun var locs -> 
          let value = CStore.M.get_unsafe store var in 
          SS.union locs (SS.union (Expr.alocs (Lit value)) (Expr.clocs (Lit value)))
        ) vars SS.empty in 
  ...
  in 
  let store_str = CStore.M.str ~which:which_store store in
  
  let heap_str = if (!noheap) then "HEAP NOT PRINTED" else (CHeap.str ~which:which_heap heap) in 
