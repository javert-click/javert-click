...
let init_specs  
  ...
  List.iter (fun (spec : Spec.t) ->
    L.log L.Verboser (lazy (Printf.sprintf "Attempting to create UP for a spec of %s : %d specs" spec.name (List.length spec.sspecs)));
    ...
    match up with 
      | Error err -> raise (Error.ThrowError (Error.EUP (Error.UPSpec (spec.name, err))))
          ...
          L.fail msg *)
      | Ok up -> 
          L.log L.Verboser (lazy (Printf.sprintf "Successfully created UP of specification of %s" spec.name));
...
let init_lemmas 
  ...
  List.iter (fun (lemma : Lemma.t) -> 
    ...
    | Error err -> raise (Error.ThrowError (Error.EUP (Error.UPLemma (lemma.name, err))))
          ...
          L.fail msg *)
    | Ok up -> 
          L.log L.Verboser (lazy (Printf.sprintf "Successfully created UP of Lemma %s" lemma.name));
...
let init_preds   
  ...
  Hashtbl.iter (fun name (pred : Pred.t) ->
    L.log L.Verboser (lazy (Printf.sprintf "Attempting to create UP of predicate %s" name)); 
    let known_params = 
      SS.of_list 
        (List.map 
    ...
    | Error err -> raise (Error.ThrowError (Error.EUP (Error.UPPred (pred.name, err))))
      ...
      L.fail msg *)
    | Ok up ->
      L.log L.Verboser (lazy (Printf.sprintf "Successfully created UP of predicate %s:\n%s" name (str up))); 
