...
module M 
  ...
  let rec get_domain ?(remove : bool option) (astate : t) (loc : Val.t) : gd_ret = 
    let f (_, astate) = get_domain ?remove:remove astate loc in 
    let remove   = Option.default false remove in  
  ...
  let simplify_formula (astate : t) (f : Formula.t) : Formula.t = 
    let state, _, _ = astate in 
    State.simplify_formula state f

  let str ?(which_store=None) ?(which_heap=None) (astate : t) : string =
    let state, preds, _ = astate in  
    let state_str = State.str ~which_store ~which_heap state in 
