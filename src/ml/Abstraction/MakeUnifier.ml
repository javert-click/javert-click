  ...
  let produce_assertion 
    ...
    match a with 
          ...
          if (Subst.mem subst x) 
            then (
              ...
              match v_x, v_le with 
               ...
               Option.map_default 
                 (fun v -> 
                   L.log L.Verboser (lazy (Printf.sprintf "update_store inside produce assertions with prog variable: %s\n" x));
      ...
      | Pure f -> 
        ...
        L.log L.Verboser (lazy (Printf.sprintf "About to assume %s in state %s" (Formula.str f') (State.str state)));
  ...
  *)
  ...
  let rec get_pred  
    ...
    match Preds.get_pred pred_pure preds pname vs_ins pred_def.ins (State.equals state) with 
      ...
      | _ -> 
        ...
        L.log L.Verboser (lazy "Recursive case - attempting to fold."); 
        let up            = pred.up in 
        ...
        (match unify ?in_unification:in_unification astate subst up with 
          | UPUSucc rets ->
            let rets = 
              List.map   
                (fun (astate', subst', _) -> 
                  L.log L.Verboser (lazy "Recursive fold success."); 
  ...
  and unify_assertion 
    ...
    | EmptyFields (le1, le2) -> 
        ...
        (match loc, domain with 
          | Some loc, Some domain -> 
              ...
              (match State.get_domain ~remove:true state loc with  
                | GDSucc [ state', domain' ] -> 
    ...
    | Pred (pname, les) ->
        ...
        L.log L.Verboser (lazy "Unifying predicate assertion"); 
        ...
        let ret = Option.map_default (unify_assertion state subst) (USucc state) cur_asrt in 
          (match ret with 
            ...
            | UFail errs -> 
                ...
                L.log L.Verboser (lazy (Printf.sprintf "WARNING: Unify Assertion Failed: %s with errors: %s" 
  ...
  and unify
    ...
    match ret with  
      ...
      | UPUFail errs when (!SCommon.unfolding && (Error.can_fix errs) && (not in_unification)) -> 
          ...
          L.log L.Verboser (lazy (Printf.sprintf "Unify. Unable to unify. Checking if there are predicates to unfold. Looking for: %s" (String.concat ", " (List.map Val.str vals)))); 
          let sp, worked = unfold_with_vals astate_i vals in 
          if (not worked) then (
            L.log L.Normal (lazy "Unify. No predicates found to unfold.");
            UPUFail errs
          ) else (
            L.log L.Verboser (lazy "Unfolding successful."); 
            let rets = 
              List.map 
                (fun (subst, astate) ->
