...
module M (Val : Val.M) : (Store.M with type vt = Val.t) = struct
  ...
  *)
  let str ?(which = None) (store : t) : string =
    let print_all = which = None in 
    let which = Option.default CCommon.SS.empty which in 
    let vars = if print_all then domain store else CCommon.SS.filter (fun x -> mem store x) which in 
    let var_vals_str = List.map (fun x -> 
      let x_val = get_unsafe store x in "(" ^ x ^ ": " ^ (Val.full_str x_val) ^ ")") (CCommon.SS.elements vars) in 
    let sep = "\n    " in
