module M 
  ...
  (Subst : Subst.M with type vt = Val.t and type t = Val.st) : 
    (Error.M with type vt = Val.t and type st = Subst.t and type event_t = string) = struct   
...
type t = 
  ...
  | ELostEvent of event_t                                     (** Lost event error: no handler added *) 
...
let str (err : t) : string = 
  match err with 
    ...
    | ELostEvent e        -> Printf.sprintf "ELostEvent(%s)" (e)
