...
let rec js_fold go_inside_funs f_ac f_state state expr =
  let new_state = f_state expr state in
  let f         = js_fold go_inside_funs f_ac f_state new_state in
  ...
  | Delete e | Unary_op (_, e) | Access (e, _) | Await e    -> f_ac (f e)
  ...
  | FunctionExp (_, _, _, s, _, _)                          -> if go_inside_funs then f_ac (f s) else f_ac []
  ...
  | Function (_, _, _, s, _)                  -> if go_inside_funs then f_ac (f s) else f_ac []
...
let rec js_map f_m expr =
  ...
  let new_e_stx =
    ...
    | FunctionExp (x, n, vs, e, a, l)-> FunctionExp (x, n, vs, f e, a, l)
...
let rec js_map_with_state f_transform f_state init_state state exp =
  ...
  let new_exp_stx, next_state =
    ...
    | FunctionExp (b, f_name, args, fb, async, lambda) ->
      let fb', state' = f state0 fb in
      FunctionExp (b, f_name, args, fb', async, lambda), state'

    | Function (b, f_name, args, fb, async) ->
      let fb', state' = f state0 fb in
      Function (b, f_name, args, fb', async), state'
    ...
    | RegExp _  -> raise (Failure "construct not supported yet REGEXP")
    ...
    | If (e, s1, s2) ->
      ...
      (* Printf.printf "s1': %s\n"
        (JSPrettyPrint.string_of_exp true s1'); *)
      If (e', s1', s2'), state'
    ...
    | With (e, s) -> raise (Failure "construct not supported yet WITH")
...
let get_all_assigned_declared_identifiers exp =
  ...
  and f is_lhs exp =
    ...
    | Throw e | Access (e, _) | Label (_, e) | Await e -> f false e
    ...
    | Await e -> (f false e)
    | bla -> raise (Failure (Printf.sprintf "Pattern matching failed, %s" (JSPrettyPrint.string_of_exp_syntax bla)))
...
let rec var_decls_inner exp =
  ...
  let f_state exp state =
    ...
    | _                          -> state) in
  js_fold true f_ac f_state true exp
...
let rec get_fun_decls exp =
  let f_ac exp state prev_state ac =
    match exp.exp_stx with
    | Function (_, _, _, _, _) -> exp :: ac
    | _ -> ac in
  js_fold false f_ac (fun x y -> y) true exp


let rec get_names_of_named_function_expressions exp : string list =
  let f_ac exp state prev_state ac =
    match exp.exp_stx with
    | FunctionExp (_, Some name, _, _, _, _) -> name :: ac
    | _ -> ac in
...
let func_decls_in_elem exp : exp list =
    match exp.exp_stx with
      | Function (s, name, args, body, _) -> [exp]
      | _ ->  []
...
let get_all_vars_f f_body f_args =
  let f_decls = get_fun_decls f_body in
  let fnames = List.map (fun f ->
    match f.exp_stx with
      | Function (s, Some name, args, body, _) -> name
      | _ -> raise (Failure ("Must be function declaration " ^ (JSPrettyPrint.string_of_exp true f)))
  ) f_decls in
  let vars = List.concat [ f_args; (var_decls f_body); fnames] in
  vars


let rec returns_empty_exp (e : JSParserSyntax.exp) =
  let get_some e =
    (match e with
    | None -> false
    | Some e -> returns_empty_exp e) in
  let rec returns_empty_exp_list (el : JSParserSyntax.exp list) =
    (match el with
    | [] -> true
    | e :: el ->
  ...
  | Null | Num _ | String _ | Bool _ | Var _ | Delete _ | Unary_op _ | BinOp _ | Access _
    ...
    | RegExp _ | FunctionExp _ | Function _ | Call _ | This | Throw _ | Return _ | Debugger | Await _ -> false
