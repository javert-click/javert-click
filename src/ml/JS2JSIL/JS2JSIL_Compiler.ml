...
let prefix_lcmds
    ...
    (cmds      : (Annot.t * string option * LabCmd.t) list) : (Annot.t * string option * LabCmd.t) list =

  let lcmds = Option.map_default (fun inv -> lcmds @ [ LCmd.SL (Invariant (inv, [])) ]) lcmds invariant in
...
let add_final_var cmds x metadata =
  ...
  | Lit lit ->
    ...
    let cmd_ass_new = (metadata, None, LBasic (Assignment (x_new, Lit lit))) in
...
let non_writable_ref_test x =
  ...
  let right_e = BinOp ((BinOp (field x, Equal, lit_str "eval")), Or, (BinOp (field x, Equal, Lit (String "arguments")))) in
  BinOp (left_e, And, right_e)

let make_unresolvable_ref_test x =
  BinOp (BinOp (base x, Equal, (Lit Null)), Or, BinOp (base x, Equal, (Lit Undefined)))
...
let make_get_value_call x err =
  ...
  | None ->
    ...
    (x_v, LCall (x_v, (Lit (String getValueName)), [ x ], Some err, None), [x_v])
...
let make_to_string_computed_call x x_v err =
  let x_s = string_var_of_var x in
  (x_s, LCall (x_s, (Lit (String toStringComputedName)), [ PVar x_v ], Some err, None))
...
let make_put_value_call x x_r err =
  ...
  (x_pv, LCall (x_pv, Lit (String putValueName), [x; PVar x_r], Some err, None))

let make_dop_call x_obj prop x_desc b err =
  let x_dop = fresh_var () in
  (x_dop, LCall (x_dop, Lit (String defineOwnPropertyName), [PVar x_obj; prop; PVar x_desc; Lit (Bool b)], Some err, None))

let make_cae_call x err =
  let x_cae = fresh_var () in
  x_cae,  LCall (x_cae, Lit (String checkAssignmentErrorsName), [ x ], Some err, None)
...
let make_create_function_object_call x_sc fun_name fun_call fun_construct params =
  let x_f = fresh_fun_var () in
  let processed_params = List.map (fun p -> Literal.String p) params in
  let fun_construct = match fun_construct with
  | None -> Lit Empty
  | Some fun_construct -> Lit (String fun_construct) in
  let cmd = LCall (x_f, Lit (String createFunctionObjectName),
    [ (PVar x_sc); Lit (String fun_name); (Lit (String fun_call)); fun_construct; (Lit (LList processed_params)) ], None, None) in
  x_f, cmd
...
let translate_named_function_literal (top_level : bool) x_sc f_name f_id params index =
    ...
    let x_f, cmd_cfoc = make_create_function_object_call x_sc f_name f_id (Some f_id) params in
    let cmd_cfoc = None, cmd_cfoc in

    (* x_er := l-nth(x_sc, index) *)
    let x_er = fresh_er_var () in
    let cmd_ass_xer = (None, (LBasic (Assignment (x_er, BinOp (PVar x_sc, LstNth, Lit (Num (float_of_int index))))))) in

    (* [x_er, f_name] := x_f *)
    (* [x_er, f_name] := {{ "d", x_f, true, true, false }} *)
    let cmd_f = if top_level
      then (None, LBasic (Mutation (PVar x_er, lit_str f_name, EList [ Lit (String "d"); PVar x_f; Lit (Bool true); Lit (Bool true); Lit (Bool false) ])))
      else (None, LBasic (Mutation (PVar x_er, lit_str f_name, PVar x_f))) in
 ...
 *)
  ...
  let cmd_ass_xr =
    ...
    LBasic (Assignment (x_r, (BinOp (PVar x_n, op,  Lit (Num 1.))))) in
...
let translate_multiplicative_binop x1 x2 x1_v x2_v aop err =
  let jsil_aop : BinOp.t =
    (match aop with
    | JSParserSyntax.Times -> Times
    | JSParserSyntax.Div   -> Div
    | JSParserSyntax.Mod   -> Mod
    | JSParserSyntax.Minus -> Minus
  ...
  (* x_r := x1_n * x2_n *)
  let x_r = fresh_var () in
  let cmd_ass_xr = LBasic (Assignment (x_r, BinOp (PVar x1_n, jsil_aop, PVar x2_n))) in
...
let translate_binop_plus x1 x2 x1_v x2_v err =
  ...
  let goto_guard_left = BinOp (UnOp (TypeOf, PVar x1_p), Equal, (Lit (Type StringType))) in
  let goto_guard_right = BinOp (UnOp (TypeOf, PVar x2_p), Equal, (Lit (Type StringType))) in
  ...
  let cmd_ass_xrthen = LBasic (Assignment (x_rthen, BinOp((PVar x1_s), StrCat, (PVar x2_s)))) in
  ...
  let cmd_ass_xrelse = LBasic (Assignment (x_relse, BinOp((PVar x1_n), Plus, (PVar x2_n)))) in
...
let translate_binop_comparison x1 x2 x1_v x2_v is_first_first flag_arg bool_undef err =
  ...
  let args =
    ...
    | false -> [ PVar x2_v; PVar x1_v ]) in
  let cmd_ac = LCall (x_ac, (Lit (String abstractComparisonName)), args @ [ Lit (Bool flag_arg) ], Some err, None) in

  (*  goto [ x_ac = undefined ] then end *)
  let then_lab = fresh_label () in
  let end_lab = fresh_label () in
  let cmd_goto = LGuardedGoto (BinOp (PVar x_ac, Equal, Lit Undefined), then_lab, end_lab) in
  ...
  let new_cmds = [
    ...
    (Some then_lab, LBasic (Assignment (x_undef, Lit (Bool bool_undef))));  (* then:  x_undef := bool_undef                                                                           *)
...
let translate_bitwise_shift x1 x2 x1_v x2_v left_fun_name right_fun_name op err =
  ...
  let cmd_fc_x1 = LCall (x1_f, (Lit (String left_fun_name)), [ PVar x1_v ], Some err, None) in

  (* x2_f := right_fun_name (x2_v) with err *)
  let x2_f = fresh_var () in
  let cmd_fc_x2 = LCall (x2_f, (Lit (String right_fun_name)), [ PVar x2_v ], Some err, None) in

  (* x_r := x1_f op x2_f *)
  let x_r = fresh_var () in
  let cmd_ass_xr = LBasic (Assignment (x_r, (BinOp (PVar x1_f, op, PVar x2_f)))) in
...
let translate_binop_equality x1 x2 x1_v x2_v non_strict non_negated err =
  ...
  let cmd_ass_xr1 = LCall (x_r1, (Lit (String f_name)), [ PVar x1_v; PVar x2_v ], Some err, None) in
  ...
  let cmd_ass_xr2, ret =
    ...
    | false ->
      ...
      (* x_r2 := (not x_r1) *)
      [ (None, LBasic (Assignment (x_r2, UnOp (Not, PVar x_r1)))) ], x_r2)) in
...
let translate_bitwise_bin_op x1 x2 x1_v x2_v bbop err =
  let bbop : BinOp.t =
    (match bbop with
    | JSParserSyntax.Bitand -> BitwiseAnd
    | JSParserSyntax.Bitor  -> BitwiseOr
    | JSParserSyntax.Bitxor -> BitwiseXor
  ...
  let x_r = fresh_var () in
  let cmd_ass_xr = LBasic (Assignment (x_r, BinOp (PVar x1_i32, bbop, PVar x2_i32))) in
...
*)
  ...
  let cmd_goto = (None, LGuardedGoto (BinOp (PVar x_new, Equal, Lit Empty), next1, next2)) in
  ...
  (* next2: x := PHI(x_new, x_previous) *)
  let x = fresh_var () in
  let cmd_phi = (Some next2, LPhiAssignment [ (x, [ (PVar x_new); (PVar x_prev) ]) ]) in
...
*)
  ...
  (* goto [ x_ret_4 = empty ] next3 next4 *)
  let cmd_goto = LGuardedGoto ((BinOp (PVar x_ret_4, Equal, Lit Empty), next3, next4)) in

  (* next4:    x_ret_5 := PHI(x_ret_4, prev_val_var) *)
  let cmd_ass_ret5 = LPhiAssignment [ (x_ret_5, [ (PVar x_ret_4); (PVar prev_val_var) ]) ] in
...
(**
  ...
  *)
let rec translate_expr tr_ctx e : ((Annot.t * (string option) * LabCmd.t) list) * Expr.t * (string list) =
  ...
  let find_var_er_index v : int option =
    ...
    | true ->
      ...
      let fid_v = (try Hashtbl.find_opt cur_var_tbl v with _ -> raise (Failure (Printf.sprintf "find_var_er_index"))) in
      Option.map (fun fid_v ->
        let fid_v_index = JS2JSIL_Preprocessing.get_vis_list_index tr_ctx.tr_vis_list fid_v in
          fid_v_index) fid_v) in


  (* All the other commands must get the offsets and nothing else *)
  let js_char_offset          = e.JSParserSyntax.exp_offset in
  ...
  let lcmds, call_lcmds, call_subst  = JS2JSIL_Preprocessing.translate_lannots_in_exp cc_tbl vis_tbl old_fun_tbl tr_ctx.tr_fid tr_ctx.tr_sc_var false e in
  ...
  let compile_var_dec x e =
    ...
    let cmd_xsf_ass = (match index with
      | Some index -> LBasic (Assignment (x_sf, BinOp (PVar tr_ctx.tr_sc_var, LstNth, lit_num (float_of_int index))))
      | None -> LCall (x_sf, lit_str dynamicScoper, [ PVar tr_ctx.tr_sc_var; lit_str x ], None, None)) in

    (* x_ref := {{ v, x_sf, "x" }}  *)
    let x_ref = fresh_var () in
    let cmd_xref_ass = LBasic (Assignment (x_ref, EList [lit_refv; PVar x_sf; lit_str x])) in
...
*)
  ...
  let translate_bin_logical_operator e1 e2 lbop err =
    ...
    let cmd_goto =
      (match lbop with
      | JSParserSyntax.And -> LGuardedGoto (PVar x1_b, next, end_lab)
      | JSParserSyntax.Or -> LGuardedGoto (PVar x1_b, end_lab, next)) in
    ...
    let cmds_args, x_args_gv, errs_args =
      List.fold_left
        (fun (cmds_args, x_args_gv, errs_args) e_arg ->
          ...
          (cmds_args @ cmds_arg @ [ (annotate_cmd cmd_gv_arg None) ], x_args_gv @ [ PVar x_arg_v ], (errs_args @ errs_arg @ errs_xarg_v)))
        ([], [], [])
        xes in
  ...
  | JSParserSyntax.Var v ->
    ...
    *)
    ...
    let translate_var_found v index =
      ...
      (match index with
      | Some _
      | None when (match index with | Some _ -> true | _ -> tr_ctx.tr_use_cc = false) ->
          let x_1 = fresh_var () in
          let cmd_ass_x1 = (match index with
            | Some index -> LBasic (Assignment (x_1, BinOp (PVar tr_ctx.tr_sc_var, LstNth, lit_num (float_of_int index))))
            | None -> LCall (x_1, lit_str dynamicScoper, [ PVar tr_ctx.tr_sc_var; lit_str v ], None, None)) in
        ...
        | _ ->
            (*  x_1 := o__hasProperty($lg, "x") with err *)
            let x_1 = fresh_var () in
              let cmd_ass_x1 = LCall (x_1, Lit (String hasPropertyName), [ Lit (Loc locGlobName); Lit (String v) ], Some tr_ctx.tr_err_lab, None) in
  ...
  *)
  | JSParserSyntax.Null ->  (annotate_first_cmd []), Lit Null, []
  | JSParserSyntax.Bool b -> (annotate_first_cmd []), Lit (Bool b), []
  | JSParserSyntax.String s ->
    let escaped_s = Str.global_replace (Str.regexp "\"") "\\\"" s in
    (annotate_first_cmd []), Lit (String escaped_s), []
  | JSParserSyntax.Num n ->  (annotate_first_cmd []), Lit (Num n), []


  (**
   Section 11.1.4 - Array Initialiser
  *)
  | JSParserSyntax.Array eos -> (* raise (Failure "not implemented yet - array literal") *)
    ...
    let cmd_cdo_call = annotate_cmd (LCall (x_cdo, Lit (String createDefaultObjectName), [ Lit (Loc locArrPrototype); Lit (String "Array") ], None, None)) None in

    (* [x_arr, "length"] := {{ "d", num, true, false, false }} *)
    let cmd_set_len num = annotate_cmd (LBasic (Mutation (PVar x_cdo,  Lit (String "length"), EList [ Lit (String "d"); Lit (Num (float_of_int num)); Lit (Bool true); Lit (Bool false); Lit (Bool false) ]))) None in
      ...
      let cmd_ass_xdesc = LBasic (Assignment (x_desc, EList [ Lit (String "d"); PVar x_v; Lit (Bool true); Lit (Bool true); Lit (Bool true) ] )) in
      ...
      List.fold_left (fun (cmds, errs, num) oe ->
        ...
        (match oe with
          | None ->
              [cmd_set_len (num + 1)], []
          | Some e ->
              translate_array_property_definition x_cdo e tr_ctx.tr_err_lab num) in
  ...
  | JSParserSyntax.Obj xs ->
    ...
    *)
    ...
    let cmd_cdo_call = annotate_cmd (LCall (x_obj, Lit (String createDefaultObjectName), [ Lit (Loc locObjPrototype) ], None, None)) None in

    let translate_property_name pname =
      (match pname with
      | JSParserSyntax.PropnameId s
        | JSParserSyntax.PropnameString s -> Lit (String s)
          | JSParserSyntax.PropnameNum n -> UnOp (ToStringOp, (Lit (Num n)))) in
    ...
    let translate_accessor_descriptor x_obj prop accessor is_getter err =
      ...
      let params =
        (match accessor.JSParserSyntax.exp_stx with
        | JSParserSyntax.FunctionExp (_, _, params, _, _, _) -> params
        | _ -> raise (Failure "getters should be annonymous functions")) in

      (* x_f := create_function_object(x_sc, f_id, f_id, params) *)
      let x_f, cmd_cfo = make_create_function_object_call tr_ctx.tr_sc_var "" f_id (Some f_id) params in
      ...
      let desc_params =
        ...
        | true ->  [ Lit (String "g"); Lit (Bool true); Lit (Bool true); Lit Empty; Lit Empty; PVar x_f; Lit Empty ]
        | false -> [ Lit (String "g"); Lit (Bool true); Lit (Bool true); Lit Empty; Lit Empty; Lit Empty; PVar x_f ] in
    ...
    let cmds, errs =
      List.fold_left (fun (cmds, errs) (pname, ptype, e) ->
        ...
        let new_cmds, new_errs =
          (match ptype with
          | JSParserSyntax.PropbodyVal -> translate_data_property_definition x_obj prop e tr_ctx.tr_err_lab
          | JSParserSyntax.PropbodyGet -> translate_accessor_descriptor x_obj prop e true tr_ctx.tr_err_lab
          | JSParserSyntax.PropbodySet -> translate_accessor_descriptor x_obj prop e false tr_ctx.tr_err_lab) in
  ...
  | JSParserSyntax.CAccess (e1, e2) ->
    ...
    (* x1_v := i__getValue (x1) with err *)
    let x1_v, cmd_gv_x1, errs_x1_v = make_get_value_call x1 tr_ctx.tr_err_lab in

    (* x2_v := i__getValue (x2) with err *)
    let x2_v, cmd_gv_x2, errs_x2_v = make_get_value_call x2 tr_ctx.tr_err_lab in

    (* x_oc := i__checkObjectCoercible (x1_v) with err *)
    let x_oc = fresh_var () in
    let cmd_coc_x1 = LCall (x_oc, Lit (String checkObjectCoercibleName), [ PVar x1_v ], Some tr_ctx.tr_err_lab, None) in
  ...
  | JSParserSyntax.Access (e, p) ->
    ...
    let x_oc = fresh_var () in
    let cmd_coc_x = LCall (x_oc, Lit (String checkObjectCoercibleName), [ PVar x_v ], Some tr_ctx.tr_err_lab, None) in

    (*  x_r := ref-o(x_v, "p") *)
    let x_r = fresh_var () in
    let cmd_ass_xr = LBasic (Assignment (x_r, EList [lit_refo; PVar x_v; lit_str p])) in
  ...
  | JSParserSyntax.New (e_f, xes) ->
    ...
    *)
    ...
    let next1 = fresh_next_label () in
    let goto_guard_expr = UnOp (Not, (BinOp (UnOp (TypeOf, PVar x_f_val), Equal, Lit (Type ObjectType)))) in
    let cmd_goto_is_obj = LGuardedGoto (goto_guard_expr, tr_ctx.tr_err_lab, next1) in
    ...
    let cmd_hf_construct = LBasic (HasField (x_hp, PVar xfvm, Lit (String _constructPropName))) in
    ...
    let cmd_get_bt = LBasic (HasField (x_bt, PVar xfvm, Lit (String "@boundThis"))) in
    ...
    let cmd_get_ba = LBasic (Lookup (x_ba, PVar xfvm, Lit (String "@boundArguments"))) in

    let x_tf = fresh_var () in
    let cmd_get_tf = LBasic (Lookup (x_tf, PVar xfvm, Lit (String "@targetFunction"))) in

    (* x_bref_fprototype := ref-o(x_tf, "prototype");  *)
    let x_bref_fprototype = fresh_var () in
    let cmd_bass_xreffprototype = LBasic (Assignment (x_bref_fprototype, EList [lit_refo; PVar x_tf; lit_str "prototype"])) in

    (* x_bf_prototype := i__getValue(x_bref_prototype) with err; *)
    let x_bf_prototype, cmd_bgv_xreffprototype, errs_bf_prototype = make_get_value_call (PVar x_bref_fprototype) tr_ctx.tr_err_lab in

    let bthen1 = fresh_then_label () in
    let belse1 = fresh_else_label () in
    let goto_guard_expr = BinOp (UnOp (TypeOf, PVar x_bf_prototype), Equal, Lit (Type ObjectType)) in
    let cmd_bis_object = LGuardedGoto (goto_guard_expr, belse1, bthen1) in

    let x_bwhyGodwhy = fresh_var () in
    let cmd_bset_proto = LBasic (Assignment (x_bwhyGodwhy, Lit (Loc locObjPrototype))) in

    let x_bprototype = fresh_var () in
    let cmd_bproto_phi = LPhiAssignment [ (x_bprototype, [ PVar x_bf_prototype; PVar x_bwhyGodwhy ]) ] in

    (* x_cdo := i__createDefaultObject (x_f_prototype); *)
    let x_bthis = fresh_this_var () in
    let cmd_bcdo_call = LCall (x_bthis, Lit (String createDefaultObjectName), [ PVar x_bprototype ], None, None) in
    ...
    let cmd_bbody = LBasic (Lookup (x_bbody, PVar xtfm, Lit (String _constructPropName))) in

    let x_bfscope = fresh_fscope_var () in
    let cmd_bscope = LBasic (Lookup (x_bfscope, PVar xtfm, Lit (String _scopePropName))) in

    let x_params = fresh_var () in
    let jsil_list_params = EList ([PVar x_bbody; PVar x_bfscope; PVar x_bthis]) in
    let cmd_append = LBasic (Assignment (x_params, (BinOp (BinOp (jsil_list_params, LstCat, PVar x_ba), LstCat, (EList x_args_gv))))) in

    let x_bconstruct = fresh_var () in
    let cmd_bind = LApply (x_bconstruct, PVar x_params, Some tr_ctx.tr_err_lab) in

    (* goto [ x_bconstruct = empty ] next3 next4; *)
    let bnext3 = fresh_next_label () in
    let bnext4 = fresh_next_label () in
    let goto_guard_expr = BinOp (UnOp (TypeOf, PVar x_bconstruct), Equal, Lit (Type ObjectType)) in
    ...
    let cmd_bphi_final = LPhiAssignment [ (x_rbind, [ PVar x_bconstruct; PVar x_bthis ]) ] in
    ...
    let cmd_ass_xreffprototype = LBasic (Assignment (x_ref_fprototype, EList [lit_refo; PVar x_f_val; lit_str "prototype"])) in

    (* x_f_prototype := i__getValue(x_ref_prototype) with err; *)
    let x_f_prototype, cmd_gv_xreffprototype, errs_xf_prototype = make_get_value_call (PVar x_ref_fprototype) tr_ctx.tr_err_lab in

    let then1 = fresh_then_label () in
    let else1 = fresh_else_label () in
    let goto_guard_expr = BinOp (UnOp (TypeOf, PVar x_f_prototype), Equal, Lit (Type ObjectType)) in
    let cmd_is_object = LGuardedGoto (goto_guard_expr, else1, then1) in

    let x_whyGodwhy = fresh_var () in
    let cmd_set_proto = LBasic (Assignment (x_whyGodwhy, Lit (Loc locObjPrototype))) in

    let x_prototype = fresh_var () in
    let cmd_proto_phi = LPhiAssignment [ (x_prototype, [ (PVar x_f_prototype); (PVar x_whyGodwhy) ]) ] in

    (* x_cdo := i__createDefaultObject (x_this, x_f_prototype); *)
    let x_cdo = fresh_var () in
    let x_this = fresh_this_var () in
    let cmd_cdo_call = LCall (x_this, Lit (String createDefaultObjectName), [ PVar x_prototype ], None, None) in

    (* x_body := [xfvm, "@construct"];  *)
    let x_body = fresh_body_var () in
    let cmd_body = LBasic (Lookup (x_body, PVar xfvm, Lit (String _constructPropName))) in
    ...
    let cmd_proc_call = LCall (x_r1, (PVar x_body), proc_args, Some tr_ctx.tr_err_lab, None) in

    (* goto [ x_r1 = empty ] next3 next4; *)
    let next3 = fresh_next_label () in
    let next4 = fresh_next_label () in
    let goto_guard_expr = BinOp (UnOp (TypeOf, PVar x_r1), Equal, Lit (Type ObjectType)) in
    ...
    let cmd_phi_final = LPhiAssignment [ (x_rcall, [ (PVar x_r1); (PVar x_this) ]) ] in

    let x_final = fresh_var () in
    let cmd_phi_join = LPhiAssignment [ (x_final, [ PVar x_rbind; PVar x_rcall ]) ] in
    ...
    @ (annotate_cmds [
      (Some call,    cmd_ass_xreffprototype);  (*        x_ref_fprototype := ref-o(x_f_val, "prototype")                        *)
  ...
  | JSParserSyntax.Call (e_f, xes)
    when (e_f.JSParserSyntax.exp_stx = (JSParserSyntax.Var js_symbolic_constructs.js_assert)) ->
      (match (List.map (fun xe -> xe.JSParserSyntax.exp_stx) xes) with
      | [ (JSParserSyntax.String assert_arg_str) ] ->
        ...
        let cmds, errs =
          List.fold_left (fun (cmds, errs) x ->
            let new_cmds, x_expr, new_errs =
              f ({ e with JSParserSyntax.exp_stx = JSParserSyntax.Var x }) in

            let x_v, cmd_gv_x, errs_x_v = make_get_value_call x_expr tr_ctx.tr_err_lab in
        ...
        let asrt =
          match Formula.lift_logic_expr le with
                    ...
                    | _ ->
                      raise (Failure (Printf.sprintf "Invalid assert. Could not lift\n%s" (Expr.str le))) in
        let cmd  = (metadata, None, (LLogic (LCmd.Assert asrt))) in

            (cmds @ [ cmd ]), Lit Empty, errs
      | es ->
                let msg = String.concat "\n" (List.map (fun e -> JSPrettyPrint.string_of_exp_syntax_1 e true) es) in
                raise (Failure (Printf.sprintf "Invalid assert:\n%s" msg))
         )

   | JSParserSyntax.Call (e_f, xes)
    when (e_f.JSParserSyntax.exp_stx = (JSParserSyntax.Var js_symbolic_constructs.js_assume)) ->
      (match (List.map (fun xe -> xe.JSParserSyntax.exp_stx) xes) with
      | [ (JSParserSyntax.String assume_arg_str) ] ->
        let e'         = Parsing_Utils.parse_expr_from_string assume_arg_str in
        let xs         = Expr.pvars e' in
        let subst      = SSubst.init [] in
        let cmds, errs =
          List.fold_left (fun (cmds, errs) x ->
        ...
        let asrt =
          match Formula.lift_logic_expr le with
                    | Some (asrt_b, _) -> asrt_b
                    | _ -> raise (Failure (Printf.sprintf "Invalid assume. Could not lift\n%s" (Expr.str le))) in
    ...
    | JSParserSyntax.Call (e_f, xes)
      when (e_f.JSParserSyntax.exp_stx = (JSParserSyntax.Var js_symbolic_constructs.js_branch)) ->
        (match (List.map (fun xe -> xe.JSParserSyntax.exp_stx) xes) with
        | [ (JSParserSyntax.String branch_arg_str) ] ->
          let e'         = Parsing_Utils.parse_expr_from_string branch_arg_str in
          let xs         = Expr.pvars e' in
          let subst      = SSubst.init [] in
          let cmds, errs =
            List.fold_left (fun (cmds, errs) x ->
          ...
          let le = SSubst.subst_in_expr subst true e' in
          let cmd  = (metadata, None, (LLogic (LCmd.Branch le))) in

              (cmds @ [ cmd ]), Lit Empty, errs
        | _ -> raise (Failure "Invalid branch!"))

    | JSParserSyntax.Call (e_f, xes)
      when (e_f.JSParserSyntax.exp_stx = (JSParserSyntax.Var js_symbolic_constructs.js_await)) ->
        (match xes with
        | [ xe ] ->

          (* C(e_arg) = cmds_farg, x_farg, errs_farg *)
          let cmds_farg, x_farg, errs_farg = f xe in

          (* x_metadata_f := metadata (x_farg) *)
          let x_metadata_f = fresh_var () in
          let cmd_metatada = LBasic (MetaData (x_metadata_f, x_farg)) in

          (* x_scope := [x_metadata_f, "@scope"] *)
          let x_scope = fresh_var () in
          let cmd_xscope = LBasic (Lookup (x_scope, PVar x_metadata_f, Lit (String _scopePropName))) in

          (* x_body := [x_f_val, "@call"]; *)
          let x_body = fresh_body_var () in
          let cmd_body = LBasic (Lookup (x_body, PVar x_metadata_f, Lit (String _callPropName))) in

          (* x_call := await(x_farg, x_scope, undefined) *)
          let x_call = fresh_var () in
          let cmd_ic = LCall (x_call, Lit (String js_symbolic_constructs.js_await), [ PVar x_body;  PVar x_scope; Lit Undefined ], None, None) in

          (* x_call := undefined *)
          let cmd_undefined = LBasic (Assignment (x_call, Lit Undefined)) in
          ...
          let cmds =
            ...
            cmds_farg @ cmds, PVar x_call, errs_farg

        | _ -> raise (Failure "Invalid await"))


  | JSParserSyntax.Call (e_f, xes)
    when (e_f.JSParserSyntax.exp_stx = (JSParserSyntax.Var js_symbolic_constructs.js_symb)) ->
      let x =
        (match (List.map (fun xe -> xe.JSParserSyntax.exp_stx) xes) with
      ...
      let cmd3 = (metadata, None, LLogic (LCmd.Assume(Not (Eq (LVar x, Lit Empty))))) in
      let cmd4 = (metadata, None, LLogic (LCmd.Assume(Not (Eq (UnOp (TypeOf, LVar x), Lit (Type ListType)))))) in
  ...
  | JSParserSyntax.Call (e_f, xes)
    when (e_f.JSParserSyntax.exp_stx = (JSParserSyntax.Var js_symbolic_constructs.js_symb_number)) ->
      ...
      let cmd1 = (metadata, None, LBasic (BCmd.Assignment (x_v, LVar x))) in
            let cmd2 = (metadata, None, LLogic (LCmd.SpecVar [ x ])) in
      let cmd3 = (metadata, None, LLogic (LCmd.AssumeType (x, Type.NumberType))) in
      [ cmd1; cmd2; cmd3 ], PVar x_v, []

  | JSParserSyntax.Call (e_f, xes)
    when (e_f.JSParserSyntax.exp_stx = (JSParserSyntax.Var js_symbolic_constructs.js_symb_string)) ->
        let x =
        (match (List.map (fun xe -> xe.JSParserSyntax.exp_stx) xes) with
        | [ JSParserSyntax.Var x ] -> "#" ^ x
        | _ -> raise (Failure "Invalid symb_string")) in
      let x_v = (fresh_var ()) ^ "_v" in
      let cmd1 = (metadata, None, LBasic (BCmd.Assignment (x_v, LVar x))) in
      let cmd2 = (metadata, None, LLogic (LCmd.SpecVar [ x ])) in
      let cmd3 = (metadata, None, LLogic (LCmd.AssumeType (x, Type.StringType))) in
      [ cmd1; cmd2; cmd3 ], PVar x_v, []

  | JSParserSyntax.Call (e_f, xes)
    when (e_f.JSParserSyntax.exp_stx = (JSParserSyntax.Var js_symbolic_constructs.js_symb_bool)) ->
        let x =
        (match (List.map (fun xe -> xe.JSParserSyntax.exp_stx) xes) with
        | [ JSParserSyntax.Var x ] -> "#" ^ x
        | _ -> raise (Failure "Invalid symb_bool")) in
      let x_v = (fresh_var ()) ^ "_v" in
      let cmd1 = (metadata, None, LBasic (BCmd.Assignment (x_v, LVar x))) in
      let cmd2 = (metadata, None, LLogic (LCmd.SpecVar [ x ])) in
  ...
  | JSParserSyntax.Call (e_f, xes) ->
    ...
    *)
    ...
    let cmd_ic = LCall (x_ic, Lit (String isCallableName), [ PVar x_f_val ], None, None) in
    ...
    let jump = if_verification call get_bt in
    let cmd_goto_is_callable = LGuardedGoto (PVar x_ic, jump, tr_ctx.tr_err_lab) in

    let xfvm = fresh_var () in
    let cmd_xfvm = LBasic (MetaData (xfvm, PVar x_f_val)) in

    let x_ibt = fresh_var () in
    let cmd_get_ibt = LBasic (HasField (x_ibt, PVar xfvm, Lit (String "@boundThis"))) in
    ...
    let x_bt = fresh_var () in
    let cmd_get_bt = LBasic (Lookup (x_bt, PVar xfvm, Lit (String "@boundThis"))) in
    ...
    let x_bbody = fresh_body_var () in
    let cmd_bbody = LBasic (Lookup (x_bbody, PVar xtfm, Lit (String _callPropName))) in

    let x_bfscope = fresh_fscope_var () in
    let cmd_bscope = LBasic (Lookup (x_bfscope, PVar xtfm, Lit (String _scopePropName))) in

    let x_params = fresh_var () in
    let jsil_list_params = EList ([PVar x_bbody; PVar x_bfscope; PVar x_bt]) in
    let cmd_append = LBasic (Assignment (x_params, (BinOp (BinOp (jsil_list_params, LstCat, PVar x_ba), LstCat, (EList x_args_gv))))) in
    ...
    let cmd_are_we_doing_eval = LGuardedGoto (BinOp (PVar x_body, Equal, (Lit (String "ExecuteEval"))), then_eval, else_eval) in

    let x_ecall = fresh_var () in
    let proc_args = (PVar JS2JSIL_Constants.var_sc_first) :: (PVar JS2JSIL_Constants.var_this) :: x_args_gv in
    let cmd_execute_eval = LECall (x_ecall, (PVar "ExecuteEval"), proc_args, Some tr_ctx.tr_err_lab) in
    ...
    let cmd_this_undefined = LBasic (Assignment (x_this_else, Lit Undefined)) in

    (* end: x_this := PHI(x_then_this, x_else_this) *)
    let x_this = fresh_this_var () in
    let cmd_ass_xthis = LPhiAssignment [ (x_this, [ (PVar x_this_then); (PVar x_this_else) ]) ] in
    ...
    let x_rcall = fresh_var () in
    let proc_args = (PVar x_fscope) :: (PVar x_this) :: x_args_gv in
    let cmd_proc_call = LCall (x_rcall, (PVar x_body), proc_args, Some tr_ctx.tr_err_lab, call_subst) in
    let x_r1 = fresh_var () in
    let cmd_phi_join = LPhiAssignment [ (x_r1, [ (PVar x_rbind); (PVar x_ecall); (PVar x_rcall) ]) ] in
    ...
    @ (if_verification [] (annotate_cmds [
      ...
      (None,            cmd_sync) ])) @ (annotate_cmds [
      (Some else_eval,  cmd_scope);            (*        x_fscope := [xfvm, "@scope"]                                              *)
      (None,            cmd_goto_obj_ref_1);   (* next2: goto [ typeOf(x_f) = ObjReference ] then else                             *)
    ...
    let errs = errs_ef @ errs_xf_val @ errs_args @ [ var_te; var_te ] @ (if_verification [] [ x_rbind ]) @ [ x_ecall; x_rcall ] in
    cmds, PVar x_r3, errs



  | JSParserSyntax.Unary_op (JSParserSyntax.Post_Incr, e) ->
    (**
      Section: 11.3.1
      C(e) = cmds, x
     ...
     *)
    let cmds, x, errs = f e in
    let new_cmds, new_errs, x_v, x_r = translate_inc_dec x true tr_ctx.tr_err_lab in
    let new_cmds = annotate_cmds new_cmds in
    (annotate_first_cmd (cmds @ new_cmds)), PVar x_v, (errs @ new_errs)


  | JSParserSyntax.Unary_op (JSParserSyntax.Post_Decr, e) ->
    (**
      Section: 11.3.2
      C(e) = cmds, x
     ...
     *)
    let cmds, x, errs = f e in
    let new_cmds, new_errs, x_v, x_r = translate_inc_dec x false tr_ctx.tr_err_lab in
    let new_cmds = annotate_cmds new_cmds in
    (annotate_first_cmd (cmds @ new_cmds)), PVar x_v, (errs @ new_errs)


  | JSParserSyntax.Delete e ->
    (**
      Section: 11.4.1
      C(e) = cmds, x
      C(delete e) =
    ...
    (* next1: goto [ ((base(x) = null) or (base(x) = undefined)) ] err next2 *)
    let cmd_goto_is_resolvable_ref = LGuardedGoto (make_unresolvable_ref_test x , tr_ctx.tr_err_lab, next3) in

    (* next2: goto [ (typeOf x) = $$v-reference_type ] err next3 *)
    let goto_guard = is_vref x in
    let cmd_goto_is_vref = LGuardedGoto (goto_guard, tr_ctx.tr_err_lab, next4) in

    (* next3: x_obj := toObject(base(x)) err *)
    let x_obj = fresh_obj_var () in
    let cmd_to_obj = LCall (x_obj, lit_str toObjectName, [ (base x) ], Some tr_ctx.tr_err_lab, None) in

    (* x_r1 := deleteProperty(x_obj, field(x), true) with err *)
    let x_r1 = fresh_var () in
    let cmd_delete = LCall (x_r1, lit_str deletePropertyName,
      [ (PVar x_obj); (field x); (Lit (Bool true)) ], Some tr_ctx.tr_err_lab, None) in
    ...
    let cmds = annotate_first_cmd (
      ...
      (None,       LGoto next6);                                                  (*        goto next6                                                               *)
      (Some next5, LBasic (Assignment (x_r2, Lit (Bool true))));              (* next5: x_r2 := true                                                             *)
      (Some next6, LPhiAssignment [ (x_r, [ (PVar x_r1); (PVar x_r2) ]) ])          (* next6: x_r := PHI(x_r1, x_r2)                                                   *)
    ])) in
    let errs = errs @ [ var_se; var_se; x_obj; x_r1 ] in
    cmds, PVar x_r, errs


  | JSParserSyntax.Unary_op (JSParserSyntax.Void, e) ->
    (** Section: 11.4.2
        C(e) = cmds, x
      C(void e) =
            cmds
    ...
    let cmds = annotate_first_cmd ( cmds @ (annotate_cmds [        (*  cmds                                *)
      ...
      (None, LBasic (Assignment (x_r, Lit Undefined)));    (*  x_r := undefined                  *)
    ])) in
    let errs = errs @ errs_x_v in
    cmds, PVar x_r, errs


  | JSParserSyntax.Unary_op (JSParserSyntax.TypeOf, e) ->
    (** Section: 11.4.3
    C(e) =  cmds, x
    C(typeof e) =
            cmds
    ...
    let x1 = fresh_var () in
    let x2 = fresh_var () in
    let cmd_gv_x = LCall (x2, (Lit (String getValueName)), [ x ], Some tr_ctx.tr_err_lab, None) in

    (* x_r := i__typeOf (x3) with err *)
    let x3 = fresh_var () in
    let x_r = fresh_var () in
    let cmd_ass_xr = LCall (x_r, (Lit (String jsTypeOfName)), [ PVar x3 ], Some tr_ctx.tr_err_lab, None) in
    ...
    let cmds = annotate_first_cmd (
      ...
      (Some next3, LBasic (Assignment (x1, Lit Undefined)));                                 (* next3:      x1 := undefined                                       *)
      (None,       LGoto next5);                                                                 (*             goto next4                                            *)
      (Some next4, cmd_gv_x);                                                                    (* next4:      x2 := getValue (x) with err                           *)
      (Some next5, LPhiAssignment [ (x3, [ (PVar x_name); (PVar x_name); (PVar x1); (PVar x2) ]) ]); (* next5:      x3 := PHI (x, x1, x2)                                 *)
  ...
  | JSParserSyntax.Unary_op (JSParserSyntax.Pre_Incr, e) ->
    (**
      Section: 11.4.4
      C(e) = cmds, x
      C(++e) =          cmds
  ...
  | JSParserSyntax.Unary_op (JSParserSyntax.Pre_Decr, e) ->
    (**
         Section: 11.4.5
         C(e) = cmds, x
      C(--e) =          cmds
  ...
  | JSParserSyntax.Unary_op (JSParserSyntax.Positive, e) ->
    (**
      Section: 11.4.6
      C(e) = cmds, x
      C(+e) =  cmds
  ...
  | JSParserSyntax.Unary_op (JSParserSyntax.Negative, e) ->
    (**
      Section: 11.4.7
      C(e) = cmds, x
      C(-e) =        cmds
  ...
  | JSParserSyntax.Unary_op (JSParserSyntax.Bitnot, e) ->
    (**
      Section: 11.4.8
      C(e) = cmds, x
      C(~e) =        cmds
    ...
    let cmds = annotate_first_cmd (
      ...
      (None, cmd_gv_x);                                                        (*  x_v := i__getValue (x) with err     *)
      (None, cmd_tn_x);                                                        (*  x_n := i__toNumber (x_v) with err   *)
      (None, LBasic (Assignment (x_i32, UnOp (ToInt32Op, PVar x_n))));        (*  x_i32 := (num_to_int32 x_n)         *)
      (None, LBasic (Assignment (x_r, UnOp (BitwiseNot, PVar x_i32))))        (*  x_r := (! x_i32)                    *)
    ])) in
    let errs = errs @ errs_x_v @ [ x_n ] in
    cmds, PVar x_r, errs


  | JSParserSyntax.Unary_op (JSParserSyntax.Not, e) ->
    (**
      Section: 11.4.9
      C(e)  =  cmds, x
      C(!e) =  cmds
  ...
  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Arith aop), e2)
    when ((aop = JSParserSyntax.Times) || (aop = JSParserSyntax.Div) || (aop = JSParserSyntax.Mod) || (aop = JSParserSyntax.Minus)) ->
    ...
    *)
    ...
    let new_cmds, new_errs, x_r = translate_multiplicative_binop x1 x2 x1_v x2_v aop tr_ctx.tr_err_lab in
  ...
  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Arith JSParserSyntax.Plus), e2) ->
    (**
      Section 11.6.1
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 + e2) =          cmds1
    ...
    *)
    ...
    let new_cmds, new_errs, x_r = translate_binop_plus x1 x2 x1_v x2_v tr_ctx.tr_err_lab in
  ...
  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Arith JSParserSyntax.Lsh), e2) ->
    (**
      Section 11.7.1
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 << e2) =    cmds1
    ...
    let new_cmds, new_errs, x_r = translate_bitwise_shift x1 x2 x1_v x2_v toInt32Name toUInt32Name LeftShift tr_ctx.tr_err_lab in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None ] @ cmds2 @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds)) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
    cmds, PVar x_r, errs


  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Arith JSParserSyntax.Rsh), e2) ->
    (**
      Section 11.7.2
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 >> e2) =    cmds1
    ...
    let new_cmds, new_errs, x_r = translate_bitwise_shift x1 x2 x1_v x2_v toInt32Name toUInt32Name SignedRightShift tr_ctx.tr_err_lab in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None ] @ cmds2 @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds)) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
    cmds, PVar x_r, errs


  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Arith JSParserSyntax.Ursh), e2) ->
    (**
      Section 11.7.3
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 >>> e2) =   cmds1
    ...
    let new_cmds, new_errs, x_r = translate_bitwise_shift x1 x2 x1_v x2_v toUInt32Name toUInt32Name UnsignedRightShift tr_ctx.tr_err_lab in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None ] @ cmds2 @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds)) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
    cmds, PVar x_r, errs


  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Comparison JSParserSyntax.Lt), e2) ->
    (**
      Section 11.8.1
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 < e2) =             cmds1
    ...
    let new_cmds, new_errs, x_r = translate_binop_comparison x1 x2 x1_v x2_v true true false tr_ctx.tr_err_lab in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None ] @ cmds2  @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds)) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
    cmds, PVar x_r, errs


  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Comparison JSParserSyntax.Gt), e2) ->
    (**
      Section 11.8.2
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 > e2) =             cmds1
    ...
    let new_cmds, new_errs, x_r = translate_binop_comparison x1 x2 x1_v x2_v false false false tr_ctx.tr_err_lab in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None ] @ cmds2 @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds)) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
    cmds, PVar x_r, errs


  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Comparison JSParserSyntax.Le), e2) ->
    (**
      Section 11.8.3
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 <= e2) =
    ...
    let new_cmds, new_errs, x_r1 = translate_binop_comparison x1 x2 x1_v x2_v false false true tr_ctx.tr_err_lab in
    let x_r2 = fresh_var () in
  ...
  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Comparison JSParserSyntax.Ge), e2) ->
    (**
      Section 11.8.4
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 >= e2) =
    ...
    let new_cmds, new_errs, x_r1 = translate_binop_comparison x1 x2 x1_v x2_v true true true tr_ctx.tr_err_lab in
    let x_r2 = fresh_var () in
    let new_cmd = LBasic (Assignment (x_r2, UnOp (Not, (PVar x_r1)))) in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None ] @ cmds2 @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds) @ [ annotate_cmd new_cmd None ]) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
  ...
  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Comparison JSParserSyntax.InstanceOf), e2) ->
    (**
      Section 11.8.6
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 instanceof e2) =
    ...
    (* goto [ (typeOf x2_v) = Obj ] next1 err *)
    let next1 = fresh_label () in
    let cmd_goto_ot = LGuardedGoto (BinOp (UnOp (TypeOf, PVar x2_v), Equal, Lit (Type ObjectType)), next1, tr_ctx.tr_err_lab) in
    ...
    let cmd_hasfield = LBasic (Lookup (x_cond, PVar x2vm, Lit (String "@class"))) in

    (* goto [ x_cond = empty ] err next2 *)
    let next2 = fresh_label () in
    let cmd_goto_xcond = LGuardedGoto (BinOp (PVar x_cond, Equal, Lit (String "Function")), next2, tr_ctx.tr_err_lab) in

    (* x_r := x_hi (x2_v, x1_v) with err *)
    let x_r = fresh_var () in
    let cmd_ass_xr = LCall (x_r, Lit (String "hasInstance"), [PVar x2_v; PVar x1_v], Some tr_ctx.tr_err_lab, None) in
  ...
  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Comparison JSParserSyntax.In), e2) ->
    (**
      Section 11.8.7
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 in e2) =
    ...
    (*  x_r := o__hasProperty (x2_v, x1_s) with err   *)
    let x_r = fresh_var () in
    let cmd_ass_xr = LCall (x_r, (Lit (String hasPropertyName)), [ PVar x2_v; PVar x1_s ], Some tr_ctx.tr_err_lab, None) in
  ...
  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Comparison JSParserSyntax.Equal), e2) ->
    (**
      Section 11.9.1
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 == e2) =
    ...
    let new_cmds, new_errs, x_r = translate_binop_equality x1 x2 x1_v x2_v true true tr_ctx.tr_err_lab in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None ] @ cmds2 @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds)) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
    cmds, PVar x_r, errs


  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Comparison JSParserSyntax.NotEqual), e2) ->
    (**
      Section 11.9.2
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 != e2) =
    ...
    let new_cmds, new_errs, x_r = translate_binop_equality x1 x2 x1_v x2_v true false tr_ctx.tr_err_lab in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None ] @ cmds2 @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds)) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
    cmds, PVar x_r, errs


  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Comparison JSParserSyntax.TripleEqual), e2) ->
    (**
      Section 11.9.4
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 === e2) =
    ...
    let new_cmds, new_errs, x_r = translate_binop_equality x1 x2 x1_v x2_v false true tr_ctx.tr_err_lab in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None] @ cmds2 @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds)) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
    cmds, PVar x_r, errs


  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Comparison JSParserSyntax.NotTripleEqual), e2) ->
    (**
      Section 11.9.5
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 !== e2) =
    ...
    let new_cmds, new_errs, x_r = translate_binop_equality x1 x2 x1_v x2_v false false tr_ctx.tr_err_lab in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None ] @ cmds2 @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds)) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
    cmds, PVar x_r, errs


  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Arith bbop), e2)
    when ((bbop = JSParserSyntax.Bitand) || (bbop = JSParserSyntax.Bitor) || (bbop = JSParserSyntax.Bitxor)) ->
    (**
      Section 11.10
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 == e2) =
    ...
    let new_cmds, new_errs, x_r = translate_bitwise_bin_op x1 x2 x1_v x2_v bbop tr_ctx.tr_err_lab in
    let cmds = annotate_first_cmd (cmds1 @ [ annotate_cmd cmd_gv_x1 None ] @ cmds2 @ [ annotate_cmd cmd_gv_x2 None ] @ (annotate_cmds new_cmds)) in
    let errs = errs1 @ errs_x1_v @ errs2 @ errs_x2_v @ new_errs in
    cmds, PVar x_r, errs


  | JSParserSyntax.BinOp (e1, (JSParserSyntax.Boolean lbop), e2) ->
    (**
      Section 11.11
      C(e1) = cmds1, x1; C(e2) = cmds2, x2
      C(e1 && e2) =
  ...
  | JSParserSyntax.ConditionalOp (e1, e2, e3) ->
    (**
      Section 11.12
    ...
    let x2_v, cmd_gv_x2, errs_x2_v = make_get_value_call x2 tr_ctx.tr_err_lab in
    (* x3_v := i__getValue (x3) with err *)
    let x3_v, cmd_gv_x3, errs_x3_v = make_get_value_call x3 tr_ctx.tr_err_lab in
    (* x_r := PHI(x2_v, x3_v) *)
    let x_r = fresh_var () in
  ...
  | JSParserSyntax.AssignOp (e1, op, e2) ->
    (**
    ...
    let new_cmds, new_errs, x_r =
      (match op with
      | JSParserSyntax.Plus -> translate_binop_plus x1 x2 x1_v x2_v tr_ctx.tr_err_lab
      | JSParserSyntax.Minus
      | JSParserSyntax.Times
      | JSParserSyntax.Div
      | JSParserSyntax.Mod -> translate_multiplicative_binop x1 x2 x1_v x2_v op tr_ctx.tr_err_lab
      | JSParserSyntax.Ursh -> translate_bitwise_shift x1 x2 x1_v x2_v toUInt32Name toUInt32Name SignedRightShift tr_ctx.tr_err_lab
      | JSParserSyntax.Lsh -> translate_bitwise_shift x1 x2 x1_v x2_v toInt32Name toUInt32Name LeftShift tr_ctx.tr_err_lab
      | JSParserSyntax.Rsh -> translate_bitwise_shift x1 x2 x1_v x2_v toInt32Name toUInt32Name UnsignedRightShift tr_ctx.tr_err_lab
      | JSParserSyntax.Bitand
          | JSParserSyntax.Bitor
          | JSParserSyntax.Bitxor -> translate_bitwise_bin_op x1 x2 x1_v x2_v op tr_ctx.tr_err_lab) in
  ...
  | JSParserSyntax.FunctionExp (_, None, params, e_body, _, lambda) ->
    (**
       Section 13
       x_f := create_function_object(x_sc, f_id, f_id, params)
    *)
    let f_id = try JS2JSIL_Preprocessing.get_codename e
      ...
      let x_f, cmd = make_create_function_object_call tr_ctx.tr_sc_var "" f_id (if lambda then None else Some f_id) params in
    let cmds =  annotate_first_cmd ([ annotate_cmd cmd None ]) in
    cmds, PVar x_f, []

  | JSParserSyntax.FunctionExp (_, Some f_name, params, _, _, _)
  | JSParserSyntax.Function (_, Some f_name, params, _, _) ->
    ...
    let cmd_ass_xfouter_meta = LBasic (New (x_f_outer_er_meta, None, Some (Lit Null))) in
    let cmd_ass_xfouter = LBasic (New (x_f_outer_er, None, Some (PVar x_f_outer_er_meta))) in

    (* [x_er, "@er"] := true *)
    let cmd_er_flag = LBasic (Mutation (PVar x_f_outer_er_meta, Lit (String _erFlagPropName), Lit (Bool true))) in

    (* x_sc_f := x_sc @ {{ x_f_outer_er }}  *)
    let x_sc_f = fresh_scope_chain_var () in
    let cmd_xscf_ass = LBasic (Assignment (x_sc_f, BinOp (PVar tr_ctx.tr_sc_var, LstCat, EList [ PVar x_f_outer_er ]))) in

    (* x_f := create_function_object(x_sc_f, f_id, params) *)
    let x_f, cmd_fun_constr = make_create_function_object_call x_sc_f f_name f_id (Some f_id) params in

    (* [x_f_outer_er, f] := x_f *)
    let cmd_fname_updt = LBasic (Mutation (PVar x_f_outer_er, Lit (String f_name), PVar x_f)) in

    (* x_cae := i__checkAssignmentErrors (ref-v(x_f_outer_er, "f")) with err *)
    let x_cae = fresh_var () in
    let cmd_cae = LCall (x_cae, lit_str checkAssignmentErrorsName, [ EList [lit_refv; PVar x_f_outer_er; lit_str f_name] ], Some tr_ctx.tr_err_lab, None) in
  ...
  | JSParserSyntax.Await e ->
      (**
        C(e) = cmds_e, x_e
        ------------------------
        C (await e) =
            cmds1
            xe_v    := i__getValue(x_e) err
            x_m     := metadata(x1_v)
            x_scope := [x_m, "@scope"]
            x_body  := [x_m, "@call"]
            x_r     := jsil_await_fun(x1_v, [x_scope, undefined])
      ...
      *)
      ...
      (* xe_v := getValue (xe) with err *)
      let xe_v, cmd_gv_xe, errs_xe_v = make_get_value_call x_e tr_ctx.tr_err_lab in

      (* x_metadata_f := metadata (xe_v) *)
      let x_metadata_f = fresh_var () in
      let cmd_metatada = LBasic (MetaData (x_metadata_f, PVar xe_v)) in
      ...
      (* x_call := await(x_farg, x_scope, undefined) *)
      let x_call = fresh_var () in
      let cmd_ic = LCall (x_call, Lit (String js_symbolic_constructs.jsil_internal_await), [ PVar x_body;  PVar x_scope; Lit Undefined ], None, None) in

      (* x_promise := getValue({{ "o", xe_v, "promise" }}) *)
      let x_promise = fresh_var () in
      let cmd_promise1 = LBasic (Assignment (x_promise, EList [ Lit (String "o"); PVar xe_v; Lit (String "promise") ])) in
      let x_promise_v, cmd_promise2, errs_promise = make_get_value_call (PVar x_promise) tr_ctx.tr_err_lab in

      (** x_value := getValue ({{ "o", x_promise, "PromiseResult" }}) *)
      let x_value = fresh_var () in
      let cmd_promise_val1 = LBasic (Assignment (x_value, EList [ Lit (String "o"); PVar x_promise_v; Lit (String "PromiseResult") ])) in
      let x_value_v, cmd_promise_val2, errs_promise_value = make_get_value_call (PVar x_value) tr_ctx.tr_err_lab in

      (** x_state := getValue ({{ "o", x_promise, "PromiseState" }}) *)
      let x_state = fresh_var () in
      let cmd_promise_state1 = LBasic (Assignment (x_state, EList [ Lit (String "o"); PVar x_promise_v; Lit (String "PromiseState") ])) in
      let x_state_v, cmd_promise_state2, errs_promise_state = make_get_value_call (PVar x_state) tr_ctx.tr_err_lab in

      (**  goto [x_state_v == ""] then end *)
      let then_lab = fresh_then_label () in
      let end_lab = fresh_end_label () in
      let cmd_goto_if = LGuardedGoto (BinOp(PVar x_state_v, Equal, Lit (String "rejected")), then_lab, end_lab) in

      (** goto err *)
      let cmd_goto_err = LGoto tr_ctx.tr_err_lab in

      (** skip *)
      let cmd_skip = LBasic Skip in

      let cmds =
        cmds_e @ annotate_cmds ([
          None, cmd_gv_xe;
          None, cmd_metatada;
          None, cmd_xscope;
          None, cmd_body;
          ...
          None, cmd_promise1;
          None, cmd_promise2;
          None, cmd_promise_val1;
          None, cmd_promise_val2;
          None, cmd_promise_state1;
          None, cmd_promise_state2;
          None, cmd_goto_if;
          Some then_lab, cmd_goto_err;
          Some end_lab, cmd_skip
        ]) in

      cmds, PVar x_value_v, errs_e @ errs_xe_v @ errs_promise @ errs_promise_value @ errs_promise_state @ [ x_value_v ]

  | JSParserSyntax.RegExp (_, _) -> raise (Failure "Not implemented: RegExp literal")
  | Function _ -> raise (Failure (Printf.sprintf "\nWeird funexp %s" (JSPrettyPrint.string_of_exp true e)))
  | x -> raise (Failure (Printf.sprintf "Unhandled expression %s at %s" (JSPrettyPrint.string_of_exp_syntax x) __LOC__))
...
and translate_statement tr_ctx e  =
  ...
  (* The first command must get the logic commands and the invariants *)
  let lcmds, _, _             = JS2JSIL_Preprocessing.translate_lannots_in_exp cc_tbl vis_tbl old_fun_tbl tr_ctx.tr_fid tr_ctx.tr_sc_var true e in
  let invariant               = JS2JSIL_Preprocessing.translate_invariant_in_exp cc_tbl vis_tbl old_fun_tbl tr_ctx.tr_fid tr_ctx.tr_sc_var e in
  let annotate_first_cmd      = prefix_lcmds lcmds invariant in
  ...
  let compile_var_dec x e =
    ...
    let cmd_xsf_ass = (match index with
      | Some index -> LBasic (Assignment (x_sf, BinOp (PVar tr_ctx.tr_sc_var, LstNth, lit_num (float_of_int index))))
      | None -> LCall (x_sf, lit_str dynamicScoper, [ PVar tr_ctx.tr_sc_var; lit_str x ], Some tr_ctx.tr_err_lab, None)) in
    ...
    (* x_cae := i__checkAssignmentErrors (x_ref) with err *)
    let x_cae = fresh_var () in
    let cmd_cae = LCall (x_cae, Lit (String checkAssignmentErrorsName), [ (PVar x_ref) ], Some tr_ctx.tr_err_lab, None) in
    ...
    | _ ->
      let x_name, cmd_new_x =
        ...
        | Lit lit ->
          ...
          let cmd_new_x = annotate_cmd (LBasic (Assignment (x_name, Lit lit))) None in
          x_name, [ cmd_new_x ]
      ...
      (cmds @ cmd_new_x @ [ cmd_ass_phi ], PVar x_ret, errs, rets, outer_breaks, conts)) in
  ...
  let make_finally_break_blocks jump_list jumps_mapping e tcf_lab end_label =
    let rec make_finally_blocks_iter jump_list finally_blocks cur_break_vars errs rets outer_breaks inner_breaks conts =
      ...
      | (js_lab, var, jump) :: rest ->
        try
          (let original_jump = try Hashtbl.find jumps_mapping jump with _ -> raise (Failure "make_finally_break_blocks") in
    ...
    let rec make_finally_blocks_iter jump_list finally_blocks errs rets outer_breaks inner_breaks conts =
      ...
      | (js_lab, var, jump) :: rest ->
        try
          ...
          (*Printf.printf ("continue processed \n");*)
          let new_loop_list = (None, end_label, tcf_lab, false) :: tr_ctx.tr_loop_list in
  ...
  let make_try_catch_cmds e1 (x, e2) catch_id =
    ...
    *)
    ...
    (* [x_er, "@er"] := true *)
    let cmd_er_flag = LBasic (Mutation (PVar x_er_meta, Lit (String _erFlagPropName), Lit (Bool true))) in

    (* x_cae := i__checkAssignmentErrors (ref-v(x_er, "x")) with err2 *)
    let x_cae = fresh_var () in
    let cmd_cae = LCall (x_cae, Lit (String checkAssignmentErrorsName), [ EList [lit_refv; PVar x_er; lit_str x] ], Some new_err2, None) in

    (* [x_er, "x"] := x_err *)
    let cmd_mutate_x = LBasic (Mutation (PVar x_er, Lit (String x), PVar x_err)) in

    (* x_sc_new := x_sc @ {{ x_er }}  *)
    let cmd_sc_updt = LBasic (Assignment (x_sc_new, BinOp (PVar tr_ctx.tr_sc_var, LstCat, EList [ PVar x_er ]))) in
  ...
  let make_try_catch_cmds_with_finally e1 (x, e2) catch_id e3 =
    ...
    *)
    ...
    let cmds = cmds1 @ (annotate_cmds [                 (*            cmds1                                                            *)
      ...
      (None,           LGoto tr_ctx.tr_err_lab);           (*            goto err                                                         *)
    ...
    cmds, PVar x_ret_4, errs, rets, breaks, conts  in
  ...
  let make_try_finally_cmds e1 e3 =
    ...
    *)
    ...
    cmds, PVar x_ret_3, errs, rets, breaks, conts  in


  (** Statements **)
  match e.JSParserSyntax.exp_stx with
  | JSParserSyntax.Script (_, es)
  | JSParserSyntax.Block es ->
    (**
     Section 12.1 - Block
    ...
    let rec loop es bprevious cmds_ac errs_ac rets_ac breaks_ac conts_ac =
      ...
      | [ e ] ->
        ...
        | true, Some x_previous ->
          ...
          let new_cmds = annotate_cmds new_cmds in
          cmds_ac @ cmds_e @ new_cmds, PVar x_r, errs_ac @ errs_e, rets_ac @ rets_e, breaks_ac @ breaks_e, conts_ac @ conts_e)
        | _, _ ->
      ...
      | e :: rest_es ->
        ...
        | true, Some x_previous ->
          (let new_cmds, x_r = make_check_empty_test x_previous x_e in
          let new_cmds = annotate_cmds new_cmds in
          loop rest_es (Some (PVar x_r)) (cmds_ac @ cmds_e @ new_cmds) (errs_ac @ errs_e) (rets_ac @ rets_e) (breaks_ac @ breaks_e) (conts_ac @ conts_e))
        | _, _ ->
  ...
  | JSParserSyntax.Null
  | JSParserSyntax.Bool _
  | JSParserSyntax.Var _
  | JSParserSyntax.This
  | JSParserSyntax.Delete _
  | JSParserSyntax.Comma _
  | JSParserSyntax.Unary_op _
  | JSParserSyntax.BinOp _
  | JSParserSyntax.Access _
  | JSParserSyntax.Call _
  | JSParserSyntax.Assign _
  | JSParserSyntax.AssignOp _
  | JSParserSyntax.FunctionExp _
  | JSParserSyntax.New _
  | JSParserSyntax.Obj _
  | JSParserSyntax.Array _
  | JSParserSyntax.CAccess _
  | JSParserSyntax.ConditionalOp _
  | JSParserSyntax.Await _ ->
    (**
     Section 12.4 - Expression Statement
     *)
    let cmds_e, x_e, errs_e = fe e in
    let x_e_v, cmd_gv_xe, errs_x_e_v = make_get_value_call x_e tr_ctx.tr_err_lab in
    let cmds = cmds_e @ [ annotate_cmd cmd_gv_xe None ] in
    let cmds = annotate_first_cmd cmds in
    cmds, PVar x_e_v, errs_e @ errs_x_e_v, [], [], []

  | JSParserSyntax.If (e1, e2, e3) ->
    (**
    ...
    let cmd_end_if = LPhiAssignment [ (x_if, [ (PVar x2_name); (PVar x3_name) ]) ] in
    ...
    let cmds = annotate_first_cmd cmds in
    let cmds, x, errs, rets, breaks, conts = cmds, PVar x_if, errs, rets2 @ rets3, breaks2 @ breaks3, conts2 @ conts3 in
    create_final_phi_cmd cmds x errs rets breaks conts break_label tr_ctx.tr_js_lab


  | JSParserSyntax.DoWhile (e1, e2) ->
    (**
    ...
    let x_ret_2 = fresh_var () in
    let x_ret_3 = fresh_var () in
    let cmd_ass_ret_1 = LPhiAssignment [ (x_ret_1, [ (PVar x_ret_0); (PVar x_ret_3) ]) ] in
    ...
    let cmd_ass_ret_3 = LPhiAssignment [ (x_ret_3, [ (PVar x_ret_1); (PVar x_ret_2) ]) ] in
  ...
  | JSParserSyntax.While (e1, e2) ->
     ...
     *)
      ...
      let expr_goto_guard = BinOp (
        ...
        Or,
        BinOp (PVar x2_v, Equal, Lit Undefined)) in
      let cmd_goto_null_undef = LGuardedGoto (expr_goto_guard, next6, next0) in

      (* x4 := "i__toObject" (x2_v) with err   *)
      let x4 = fresh_var () in
      let cmd_to_obj_call = LCall (x4, Lit (String toObjectName), [ PVar x2_v ], Some tr_ctx.tr_err_lab, None) in

      (* xlf := "i__getAllEnumerableFields" (x4)  with err  *)
      let xlf = fresh_var () in
      let cmd_get_enum_fields = LCall (xlf, Lit (String getEnumFieldsName), [ PVar x4 ], Some tr_ctx.tr_err_lab, None) in
      ...
      let cmd_ass_xret1 = LPhiAssignment [ (x_ret_1, [ (PVar x_ret_0); (PVar x_ret_3) ]); (x_c_1, [ (PVar x_c); (PVar x_c_2) ]) ] in

      (* goto [x_c_1 < len] body end_loop  *)
      let cmd_goto_len = LGuardedGoto (BinOp (PVar x_c_1, LessThan, PVar len), body, end_loop) in

      (* xp := l-nth (xf, x_c_1)  *)
      let xp = fresh_var () in
      let cmd_ass_xp = LBasic (Assignment (xp, BinOp (PVar xf, LstNth, PVar x_c_1))) in
      ...
      let cmd_ass_xxl = LBasic (Assignment (xxl, BinOp (PVar xl, LstNth, Lit (Num 1.)))) in

      (*  xhf := hasField (xxl, xp) *)
      let xhf = fresh_var () in
      let cmd_ass_hf = LBasic (HasField (xhf, PVar xxl, PVar xp)) in
      ...
      (* x_ret_3 := PHI(x_ret_1, x_ret_2) *)
      let cmd_phi_xret3 = LPhiAssignment [ (x_ret_3, [ (PVar x_ret_1); (PVar x_ret_1); (PVar x_ret_2) ]) ] in

      (* x_c_2 := x_c_1 + 1 *)
      let cmd_ass_incr = LBasic (Assignment (x_c_2, BinOp (PVar x_c_1, Plus, Lit (Num 1.)))) in
      ...
      let cmd_goto_xret4_empty = LGuardedGoto (
        BinOp (PVar x_ret_4, Equal, Lit Empty), next5, next6) in

      (* x_ret_5 := PHI(x_ret_0, x_ret_1, x_ret_4) *)
      let cmd_phi_xret5 = LPhiAssignment [ (x_ret_5, [ (PVar x_ret_0); (PVar x_ret_1); (PVar x_ret_4) ]) ] in
    ...
    | JSParserSyntax.For (e1, e2, e3, e4) ->
    (**
     Section 12.6.3
    ...
    let cmds1, x1, errs1 =
      ...
      | None ->
        ...
        [ annotate_cmd cmd_ass_x1v None ], PVar x1_v, []) in
    (* x1_v := i__getValue (x1) with err *)
    let x1_v, cmd_gv_x1, errs_x1_v = make_get_value_call x1 tr_ctx.tr_err_lab in
    ...
    let cmds2, x2, errs2 =
      ...
      | None ->
        let x2 = fresh_var () in
        let cmd_ass_x2 = annotate_cmd (LBasic (Assignment (x2, Lit (Bool true)))) None in
        [ cmd_ass_x2 ], PVar x2, []) in
    ...
    let cmds3, _, errs3 =
      ...
      | None ->
        ...
        [ annotate_cmd cmd_ass_x3v None ], PVar x3_v, []) in
    ...
    (* x4_v := i__getValue (x4) with err *)
    let x4_v, cmd_gv_x4, errs_x4_v = make_get_value_call x4 tr_ctx.tr_err_lab in
  ...
  | JSParserSyntax.Return e ->
    ...
    *)
    ...
    | None ->
      let x_r = fresh_var () in
      (* x_r := undefined *)
      let cmd_xr_ass = annotate_cmd (LBasic (Assignment (x_r, Lit Undefined))) None in
      (* goto lab_ret *)
    ...
    | Some e ->
      ...
      (* x_r := i__getValue(x) with err *)
      let x_r, cmd_gv_x, errs_x_r = make_get_value_call x tr_ctx.tr_err_lab in
  ...
  | JSParserSyntax.Try (e1, Some (x, e2), Some e3) ->
    (**
  ...
  | JSParserSyntax.Try (e1, None, Some e3) ->
    (**
     Section 12.14 - The try Statement
  ...
  | JSParserSyntax.Try (e1, Some (x, e2), None) ->
    (**
     Section 12.14 - The try Statement
     C(e1) = cmds1, x1; C(e2) = cmds2, x2;
     -----------------------------------------------------------
  ...
  | JSParserSyntax.Switch (e, xs) ->
    ...
    let compile_case e s x_prev_found x_prev_case x_switch_guard end_switch js_lab fresh_end_case_label =
      ...
      (* goto [ not x_prev_found ] next1 next2 *)
      let cmd_goto_1 = LGuardedGoto ( UnOp(Not, PVar x_prev_found), next1, next2) in
      ...
      let cmd_goto_2 = LGuardedGoto ( BinOp(PVar x1_v, Equal, PVar x_switch_guard), next2, end_case) in

      (* x_found_2 := PHI(x_false, x_true)  *)
      let x_case = fresh_case_var () in
      let cmd_ass_xfound = LPhiAssignment [ (x_found, [ (Lit (Bool false)); (Lit (Bool true)) ]); (x_case, [ (PVar x_prev_case); (PVar x2) ]) ] in
    ...
    let compile_default s b_stmts x_old_b x_found_b end_switch js_lab cur_breaks_ab =
      ...
      (* goto [ not (x_found_b) ] next end_switch *)
      let next = fresh_next_label () in
      let cmd_goto = LGuardedGoto( UnOp( Not, PVar x_found_b), next, end_switch) in
    ...
    let filter_cases cases =
      List.fold_left (fun (a_cases, def, b_cases) case ->
        (match case, def with
        | (JSParserSyntax.Case e, s), None -> (((e, s) :: a_cases), def, b_cases)
        | (JSParserSyntax.DefaultCase, s), None -> (a_cases, Some s, b_cases)
        | (JSParserSyntax.Case e, s), Some _ -> (a_cases, def, ((e, s) :: b_cases))
        | (JSParserSyntax.DefaultCase, _), Some _ -> raise (Failure "No two defaults for the same try")))
      ([], None, [])
      cases in
    ...
    (* x_guard_v := i__getValue (x_guard) with err  *)
    let x_guard_v, cmd_gv_xguardv, errs_x_guard_v = make_get_value_call x_guard tr_ctx.tr_err_lab in
    let cmd_gv_xguardv = annotate_cmd cmd_gv_xguardv None in
    (* x_found := false *)
    let cmd_x_found_init = annotate_cmd (LBasic (Assignment (x_found_init, Lit (Bool false)))) None in
    (* x_init_val := empty *)
    let x_init = fresh_var () in
    let cmd_val_init = annotate_cmd (LBasic (Assignment (x_init, Lit Empty))) None in
    ...
    | [], Some def ->
      ...
      let cmds = annotate_first_cmd cmds in
      cmds, PVar x_r, errs_as @ errs_def, rets_as @ rets_def, outer_breaks_as @ outer_breaks_def, conts_as @ conts_def
    ...
    | _, Some def ->
      ...
      (* goto [ x_found_a ] default b_cases *)
      let cmd_goto_xfounda = (annotate_cmd (LGuardedGoto (PVar x_found_as, default_lab, b_cases_lab)) None) in

      (* default:   x_found_b := PHI(x_false, x_found_b) *)
      let x_found_b = fresh_xfoundb_var () in
      let cmd_ass_xfoundb = (annotate_cmd (LPhiAssignment [ (x_found_b, [ (Lit (Bool false)); (PVar x_found_bs) ]) ]) (Some default_lab)) in
      ...
      let cmds = annotate_first_cmd cmds in
      cmds, PVar x_def, errs_as @ errs_bs @ errs_def, rets_as @ rets_bs @ rets_def, outer_breaks_as @ outer_breaks_bs @ outer_breaks_def, conts_as @ conts_bs @ conts_def

    | _, _ -> raise (Failure "no b cases with no default"))


  | JSParserSyntax.Function (_, n, params, e_body, _) -> [], Lit Empty, [], [], [], []

  | JSParserSyntax.With (_, _) -> raise (Failure "Not implemented: with (this should not happen)")
  | JSParserSyntax.RegExp (_, _) -> raise (Failure "Not implemented: RegExp literal")
...
let translate_fun_decls (top_level : bool) (sc_var : string) (cur_index : int) e  =
  ...
  let hoisted_fdecls =
    List.fold_left (fun ac f_decl ->
      let f_name, f_params =
        (match f_decl.JSParserSyntax.exp_stx with
        | JSParserSyntax.Function (s, Some f_name, f_params, body, _) -> f_name, f_params
...
let generate_main offset_converter e spec main_name : EProc.t =
  ...
  (* x_sc := {{ $lg }} *)
  let init_scope_chain_ass = annotate_cmd (LBasic (Assignment (sc_var_main, Lit (LList [ Loc locGlobName ])))) None in

  let sc_var_main = JS2JSIL_Constants.var_sc_first in

  (* x_sc_fst := {{ $lg }} *)
  let init_scope_chain_ass_again = annotate_cmd (LBasic (Assignment (sc_var_main, Lit (LList [ Loc locGlobName ])))) None in

  (* __this := $lg *)
  let this_ass = annotate_cmd (LBasic (Assignment (var_this, Lit (Loc locGlobName)))) None in
  ...
  let global_var_asses =
    List.map
      (fun global_v ->
        (annotate_cmd
          (LBasic (Mutation(Lit (Loc locGlobName),  Lit (String global_v), Lit (LList [(String "d"); Undefined; (Bool true); (Bool true); (Bool false)]))))
          None))
      (var_decls e) in
  ...
  let ctx = make_translation_ctx offset_converter main_name [ main_name ] sc_var_main in
  let cmds_hoist_fdecls = translate_fun_decls true sc_var_main 0 e in
  let cmds_hoist_fdecls = annotate_cmds_top_level (Annot.init ()) cmds_hoist_fdecls in
  ...
  (* x_ret := x_e *)
  let ret_ass = annotate_cmd (LBasic (Assignment (Flag.return_variable, x_e))) None in
  ...
  let cmd_err_phi_node = make_final_cmd errs ctx.tr_err_lab Flag.return_variable in
  let lab_err_cmd = annotate_cmd LReturnError None in
  let global_err_asrt = annotate_cmd (LLogic (LCmd.Assert False)) None in
  let err_cmds =
    if !symb_testing
      then [ cmd_err_phi_node; global_err_asrt; lab_err_cmd ]
...
let generate_proc_eval new_fid ?use_cc e vis_fid : EProc.t =
  let annotate_cmd cmd lab = (Annot.init (), lab, cmd) in
  let annotate_cmds cmds =
    List.map
      (fun (lab, cmd) ->
  ...
  let cmd_er_m_creation = annotate_cmd (LBasic (New (x_erm, None, Some (Lit Null)))) None in
  let cmd_er_creation = annotate_cmd (LBasic (New (x_er, None, Some (PVar x_erm)))) None in

  (* [x_er, "@er"] := true *)
    let cmd_er_flag = annotate_cmd (LBasic (Mutation (PVar x_erm, Lit (String _erFlagPropName), Lit (Bool true)))) None in

  (* [x_er, decl_var_i] := undefined *)
  let new_fid_vars = var_decls e in
  let cmds_decls =
    List.map (fun decl_var ->
      let cmd = LBasic (Mutation (PVar x_er, Lit (String decl_var), Lit Undefined)) in
      (annotate_cmd cmd None))
    new_fid_vars in

  (* x_sc_0 = x_scope @ {{ x_er }}  *)
  let cmd_ass_er_to_sc = annotate_cmd  (LBasic (Assignment (var_sc_proc, (BinOp ((PVar var_scope), LstCat, EList [ (PVar var_er) ]))))) None in
  ...
  let ctx = update_tr_ctx ?use_cc:use_cc ctx in
  ...
  (* x_ret := x_e *)
  let ret_ass = annotate_cmd (LBasic (Assignment (Flag.return_variable, PVar xe_v))) None in

  (* lab_ret: skip *)
  let lab_ret_cmd = annotate_cmd LReturnNormal None in

  (* lab_err: x_error := PHI(errs, x_fake_ret) *)
  let cmd_error_phi = make_final_cmd (errs @ errs_xe_v) ctx.tr_err_lab Flag.return_variable in
  let lab_err_cmd = annotate_cmd LReturnError None in

  let fid_cmds =
    [ cmd_er_m_creation; cmd_er_creation; cmd_er_flag ] @ cmds_decls @ [ cmd_ass_er_to_sc; cmd_ass_te; cmd_ass_se ] @ cmds_hoist_fdecls @ cmds_e
    @ [ cmd_gv_xe; ret_ass; lab_ret_cmd; cmd_error_phi; lab_err_cmd] in
  {
    name = new_fid;
...
let generate_proc ?use_cc offset_converter e fid params vis_fid spec has_jsil_annot : EProc.t =
  ...
  let new_ctx = { ctx with tr_ret_lab = ("pre_" ^ ctx.tr_ret_lab); tr_err_lab = ("pre_" ^ ctx.tr_err_lab) } in
  let cmds_hoist_fdecls = translate_fun_decls false var_sc_proc ((List.length vis_fid)-1) e in
  let cmds_hoist_fdecls = annotate_cmds_top_level (Annot.init ()) cmds_hoist_fdecls in

  let number_undefineds = (List.length vis_fid) -1 in
  let rec create_undefineds number =
    if (number = 0) then
    []
    else
    create_undefineds (number -1) @ [(Lit Undefined)] in
  let undefineds = create_undefineds number_undefineds in


  (* JSIL Only calls *)

  (* x__scope := {{undefined, undefined, ...}}*)
  let undefined_scopes = EList undefineds in
  let cmd_scope_undefined = annotate_cmd (LBasic (Assignment (var_scope, undefined_scopes))) None in
  (* x__this := undefined*)
  let cmd_this_undefined = annotate_cmd (LBasic (Assignment (var_this, Lit Undefined))) None in

  (* x_er_m := new (null)   *)
  (* x_er   := new (x_er_m) *)
  let cmd_er_m_creation = annotate_cmd (LBasic (New (var_er_metadata, None, Some (Lit Null)))) None in
  let cmd_er_creation = annotate_cmd (LBasic (New (var_er, None, Some (PVar var_er_metadata)))) None in

  (* [x_er, "@er"] := true *)
    let cmd_er_flag = annotate_cmd (LBasic (Mutation (PVar var_er_metadata, Lit (String _erFlagPropName), Lit (Bool true)))) None in

  (* [x_er, "arg_i"] := x_{i+2} *)
  let cmds_params =
    List.map (fun param ->
      let cmd = LBasic (Mutation (PVar var_er, Lit (String param), PVar param)) in
      (annotate_cmd cmd None))
    params in
  ...
  let cmds_decls =
    List.map (fun decl_var ->
      let cmd = LBasic (Mutation (PVar var_er, Lit (String decl_var), Lit Undefined)) in
      (annotate_cmd cmd None))
    (var_decls e) in
  ...
  *)
  ...
  let cmd_dr_ass = annotate_cmd (LBasic (Assignment (x_dr, Lit Empty))) None in
  let rets = rets @ [ x_dr ] in

  (* x_scope_final := x_scope *)
  let cmd_xscope_final = annotate_cmd (LBasic (Assignment (JS2JSIL_Constants.var_scope_final, PVar var_sc_proc))) None in

  (* pre_lab_ret: x_return := PHI(...) *)
  let cmd_return_phi = make_final_cmd rets new_ctx.tr_ret_lab Flag.return_variable in

  let x_ignore      = fresh_var () in
  (* let cmd_del_errs  = annotate_cmd (LCall (x_ignore, Lit (String JS2JSIL_Constants.deleteErrorObjects), [ PVar Flag.return_variable; PVar var_te; PVar var_se ], None, None)) None in *)
  ...
  let errs = errs in
  let cmd_error_phi = make_final_cmd errs new_ctx.tr_err_lab Flag.return_variable in
...
(**** EVAL ****)
let js2jsil_eval (from_jsil : bool) (prog : Prog.t) fid_parent e =
  ...
  let e, fid_eval, vislist_eval, eval_fun_tbl =
    if (not from_jsil)
      then JS2JSIL_Preprocessing.preprocess_eval cc_tbl vis_tbl e fid_parent []
      else
        (let proc_eval_id, eval_fun_tbl = JS2JSIL_Preprocessing.preprocess_eval_jsil e  in
        e, proc_eval_id, [], eval_fun_tbl) in
      ...
      Option.may
        (fun f_body ->
          let proc =
            (if (f_id = fid_eval)
              then (
                generate_proc_eval fid_eval ?use_cc:(Some false) e vislist_eval)
              else
                (let vislist =
                ...
                let f_params =
                  ...
                  | "x__scope" :: rest -> rest
                  | _ -> f_params) in
                generate_proc offset_converter f_body f_id f_params vislist None false)) in
          let proc = EProc.to_proc proc in
          Hashtbl.add prog f_id proc;
          PreProcessing_Utils.extend_which_pred which_pred proc) f_body)
    eval_fun_tbl;

  let proc_eval = try Hashtbl.find prog fid_eval with _ ->
      raise (Failure "no eval proc was created") in
    proc_eval.name


(* FUNCTION CONSTRUCTOR *)
let js2jsil_function_constructor_prop (prog : Prog.t) fid_parent params e =
  let main_fid = Prog.get_main prog in
  let prog, which_pred = prog.procs, prog.predecessors in

  let offset_converter x = 0 in

  let e, new_fid, vislist, new_fun_tbl = JS2JSIL_Preprocessing.preprocess_eval cc_tbl vis_tbl e main_fid params in
    ...
    (fun f_id (_, f_params, f_body, _, _) ->
      Option.may
        ...
        let proc =
            ...
            generate_proc offset_converter f_body f_id f_params vis_fid None false) in
...
let js2jsil e offset_converter for_verification filename =
  let main_name = main_fid ^ filename in
  let e, only_specs, predicates, ids = JS2JSIL_Preprocessing.preprocess cc_tbl fun_tbl vis_tbl e (main_name) in
  let procedures = Hashtbl.create medium_tbl_size in
  let top_annots = JS2JSIL_Preprocessing.get_top_level_annot e in
  Hashtbl.iter
    (fun f_id (_, f_params, f_body, spec, annotations) ->
      Option.map_default
        (fun f_body ->
          (* print_normal (Printf.sprintf "Procedure %s is recursive?! %b" f_id f_rec); *)
          let proc =
            (if (f_id = main_name)
              then generate_main offset_converter e spec main_name
              else
                (let vis_fid =
                  try Hashtbl.find vis_tbl f_id
                    with _ ->
  ...
  let imports = compute_imports for_verification in

  let lemmas  = Lemma.init_tbl () in
  let macros  = Macro.init_tbl () in
  let bispecs = BiSpec.init_tbl () in
  (EProg.init imports lemmas predicates only_specs procedures macros bispecs (ids @ ([main_name])) filename), cc_tbl, vis_tbl
