...
let concretise2 (a : Formula.t) (x: string) (y: string) (les : Expr.t list) : Formula.t list =
  List.map 
    (fun (le1, le2) -> Formula.substitution (SSubst.init [ x, le1; y, le2]) true a) 
    (cross_product les les (fun x y -> (x, y)))
...
let make_global_axioms 
  ...
  let lvar_y = Expr.LVar y_name in  
  ...
  let llen2 : Formula.t = 
    Or (Eq (lvar_x, Lit (LList [])), 
       Less (Lit (Num 0.), UnOp (LstLen, lvar_x))) in 
  let llen2_s = concretise llen2 x_name list_vars in 

  (* forall x. (car(x) = l-nth(x, 0) *)
  let carlnth0 : Formula.t = Eq (UnOp (Car, lvar_x), BinOp (lvar_x, LstNth, Lit (Num 0.))) in 
  ...
  let rev1 : Formula.t = 
    Or (Eq (lvar_x, Lit (LList [])), 
       Not (Eq (Lit (LList []), UnOp (LstRev, lvar_x)))) in 
  ...
  rev0_s @ rev1_s @ slen1_s @ llen1_s @ llen2_s @ carlnth0_s @ lstcat1_s @ cons_nil_idem_r_s
...
let make_list_axioms (le : Expr.t) : Formula.t list =
  let rec loop_nth original_les les i axioms =
    ...
    | []               -> axioms
    | le :: rest_les   ->
      let cur_axiom : Formula.t = Eq ((BinOp (EList original_les, LstNth, Lit (Num (float_of_int i)))), le) in
      loop_nth original_les rest_les (i+1) (cur_axiom :: axioms) in


  match (le : Expr.t) with
  | EList les ->
    let len_axiom : Formula.t  = Eq (UnOp (LstLen, le), Lit (Num (float_of_int (List.length les)))) in
    let nth_axioms = loop_nth les les 0 [] in
    len_axiom :: nth_axioms
    
  | BinOp (le_hd, LstCons, le_tail) ->
    let len_axiom : Formula.t  = 
      Eq (UnOp (LstLen, le), 
        BinOp (Lit (Num (float_of_int 1)), Plus, UnOp (LstLen, le_tail))) in
      let nth_axiom : Formula.t = 
        Eq ((BinOp (le, LstNth, Lit (Num (float_of_int 0)))), le_hd) in
      [ len_axiom; nth_axiom ]
...
let make_string_axioms (s : string) : Formula.t list =
  ...
  let rec loop_nth chars i axioms =
    ...
    | []               -> axioms
    | c :: rest_chars   ->
      let cur_axiom : Formula.t = Eq ((BinOp (Lit (String s), StrNth, Lit (Num (float_of_int i)))), Lit (String c)) in
      loop_nth rest_chars (i+1) (cur_axiom :: axioms) in

  let slen_axiom : Formula.t = Eq (UnOp (StrLen, (Lit (String s))), Lit (Num (float_of_int (String.length s)))) in
...
let make_relevant_axioms 
  ...
  let a_strings    = CCommon.remove_string_duplicates a_strings in
...
let get_axioms assertions gamma = 
  (* Get list variables *)
    let list_vars   = List.map (fun x : Expr.t -> LVar x) (TypEnv.get_vars_of_type gamma ListType) in 
  (* Get string variables *)
  let string_vars = List.map (fun x : Expr.t -> LVar x) (TypEnv.get_vars_of_type gamma StringType) in
...
let simplify_pfs_and_gamma 
    (fs     : Formula.t list)
    (gamma  : TypEnv.t) : Formula.Set.t * TypEnv.t * SSubst.t = 
  
  let gamma          = TypEnv.copy gamma in 
  let pfs            = PFS.of_list fs in 
  let (subst, _)     = Simplifications.simplify_pfs_and_gamma gamma pfs None in
  let fs_lst         = PFS.to_list pfs in 
  let fs_set         = Formula.Set.of_list fs_lst in  
  fs_set, gamma, subst

  let simplify_for_sat_check 
    (fs     : Formula.Set.t)
    (gamma  : TypEnv.t) : Formula.Set.t =
  let gamma  = TypEnv.copy gamma in 
  let pfs    = PFS.of_list (Formula.Set.elements fs) in 
  let _      = Simplifications.simplify_for_sat_check gamma pfs in
  let fs_lst = PFS.to_list pfs in 
  let fs_set = Formula.Set.of_list fs_lst in  
    fs_set
  
let check_satisfiability_with_model 
    (fs         : Formula.t list) 
    (gamma      : TypEnv.t) : SSubst.t option =

  let fs, gamma, subst = simplify_pfs_and_gamma fs gamma in
  let fs               = simplify_for_sat_check fs gamma in 
  let model            = Z3Encoding.check_sat_core fs gamma in 
  let lvars            = List.fold_left (fun ac vs -> SS.union ac vs) SS.empty (List.map Formula.lvars (Formula.Set.elements fs)) in 
  let z3_vars          = SS.diff lvars (SSubst.domain subst None) in 
  let ident_subst_vars = SSubst.init (List.map (fun x -> x, Expr.LVar x) (SS.elements z3_vars)) in 
  L.log L.Verboser (lazy (Printf.sprintf "OBTAINED VARS: %s" (String.concat ", " (SS.elements z3_vars)))); 
  (match model with
  | None -> None 
  | Some model -> try (
      SSubst.merge_left subst ident_subst_vars;
      let subst' = Z3Encoding.lift_z3_model model gamma z3_vars in 
      L.log L.Verboser (lazy (Printf.sprintf "Lifted variables:\n%s" (SSubst.str subst'))); 
      let pfs = PFS.of_list (Formula.Set.elements fs) in
      PFS.substitution_in_place subst' pfs;
      let (pfs, _, _) = simplify_pfs_and_gamma (PFS.to_list pfs) gamma in 
      if not (Formula.Set.mem False pfs) then 
        SSubst.merge_left subst subst';
      Some subst) with _ -> None)
...
let check_satisfiability 
  ...
  let t = time() in 
  L.log L.Verboser (lazy "Entering FOSolver.check_satisfiability");
  let fs, gamma, _ = simplify_pfs_and_gamma fs gamma in 
  let fs           = simplify_for_sat_check fs gamma in 
...
let check_entailment 
  ...
  let msg = lazy (Printf.sprintf "Preparing entailment check:\nExistentials:\n%s\nLeft:%s\nRight:%s\nGamma:\n%s\n"
       (String.concat ", " (SS.elements existentials))
       (PFS.str (DynArray.of_list left_fs))
       (PFS.str (DynArray.of_list right_fs))
       (TypEnv.str gamma)) in 
    L.log L.Verboser msg;
  ...
  let existentials = Simplifications.simplify_implication existentials left_fs right_fs gamma in
  ...
  (* If right side is false, return false *)
    ...
    let left_sat = Z3Encoding.check_sat (Formula.Set.of_list left_fs) gamma_left in 
    
    (* If the right side is empty or left side is not satisfiable, return the result of 
       checking left-side satisfiability *)
    if ((List.length right_fs = 0) || not left_sat) then left_sat else (
      ...
      let right_fs = List.map (fun f : Formula.t -> Formula.push_in_negations (Not f)) right_fs in 
      let right_f : Formula.t = 
        if (SS.is_empty existentials) then Formula.disjunct right_fs else (
          let binders = List.map (fun x -> (x, TypEnv.get gamma_right x)) (SS.elements existentials) in 
          ForAll (binders, Formula.disjunct right_fs)
        ) in 
      
      let msg = lazy (Printf.sprintf "Encoded the entailment as the following SAT question:%s\nGamma:\n%s\n"
         (String.concat ", " (List.map Formula.str (right_f :: (left_fs @ [] (* axioms *)))))
         (TypEnv.str gamma_left)) in 
      L.log L.Verboser msg; 
      let ret = Z3Encoding.check_sat (Formula.Set.of_list ((right_f :: (left_fs @ [] (* axioms *))))) gamma_left in  
      L.log L.Verboser (lazy (Printf.sprintf "Entailment returned %b" (not ret)));
      (* update_statistics ("FOS: CheckEntailment: " ^ caller) (time() -. t); *)
      not ret 
    )
  )
...
| true -> Some true
| false -> (match (e1 : Expr.t), (e2 : Expr.t) with

  | Lit (String str), LVar x
  | LVar x, Lit (String str) ->
    if (String.get str 0 = '@')
      then if ((List.mem (Formula.Not (Eq (BinOp (LVar x, StrNth, Lit (Num 0.)), Lit (String "@")))) pfs)  ||
               (List.mem (Formula.Not (Eq (Lit (String "@"), BinOp (LVar x, StrNth, Lit (Num 0.))))) pfs))
        then Some false
        else None
      else None
  ...
  (* ELists *)
  | Lit (LList _), EList _
  | EList _, Lit (LList _) -> None
  | Lit _, EList _
  | EList _, Lit _ -> Some false

  (* other *)
  | _, _ -> None))

let rec separate_and (pf : Formula.t): Formula.t list = 
  let f = separate_and in 
  match pf with 
  | And (f1, f2) -> (f f1) @ (f f2)
  | _ -> [ pf ]

let is_equal e1 e2 pfs gamma caller =
  let feq = Simplifications.reduce_formula ?gamma:(Some gamma) ?pfs:(Some pfs) (Eq (e1, e2)) in
  let result = (match feq with
  | True  -> true
  | False -> false 
  | Eq _ -> check_entailment SS.empty (PFS.to_list pfs) [ feq ] gamma ("is_equal: " ^ caller)
  | And _ -> 
      let rpfs = separate_and feq in 
      check_entailment SS.empty (PFS.to_list pfs) rpfs gamma ("is_equal: " ^ caller)
  | _ -> raise (Failure ("Equality reduced to something unexpected: " ^ (Formula.str feq)))
  ) in 
    result

let is_different e1 e2 pfs gamma =
  let feq = Simplifications.reduce_formula ?gamma:(Some gamma) ?pfs:(Some pfs) (Not (Eq (e1, e2))) in
  let result = (match feq with
  | True  -> true
  | False -> false 
  | Not _ -> 
      check_entailment SS.empty (PFS.to_list pfs) [ feq ] gamma "is_different"
  | _ -> raise (Failure ("Inequality reduced to something unexpected: " ^ (Formula.str feq)))
  ) in 
    result
