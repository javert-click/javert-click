...
let rec infer_types_to_gamma 
  ...
  | BinOp (le1, op, le2) ->
    let (rqt1 : Type.t option), (rqt2 : Type.t option), (rt : Type.t option) =
      ...
      | StrNth         ->  Some StringType,  Some NumberType, None
  ...
  | BinOp (e1, op, e2) ->
    ...
    | true, true -> 
      ...
      | StrNth ->
        ...
        (match success with
        | false -> CCommon.print_to_all (Printf.sprintf "%s not a string" (Expr.str e1)); def_neg
        | true -> 
          let _, success, _ = infer_type e2 NumberType constraints in
          (match success with
          | false -> CCommon.print_to_all (Printf.sprintf "%s not a number" (Expr.str e2)); def_neg
          | true -> 
            let new_constraint1 : Formula.t = (LessEq (Lit (Num 0.), e2)) in
