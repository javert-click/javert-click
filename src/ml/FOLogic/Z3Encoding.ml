...
let encode_quantifier quantifier_type ctx quantified_vars var_sorts assertion =
  if ((List.length quantified_vars) > 0) then
    (let quantified_assertion =
      Quantifier.mk_quantifier_const
        ctx
        quantifier_type
...
let rec encode_assertion (a : Formula.t) : ZExpr.expr =
  ...
  | LessEq (le1, le2) ->
    ...
    mk_le ctx le1' le2'
  | StrLess (_, _)    -> 
      print_to_all (Formula.str a);
      raise (Failure ("Z3 encoding does not support STRLESS"))
  | True               -> Boolean.mk_true ctx
...
let check_sat_core (fs : Formula.Set.t) (gamma : TypEnv.t) : Model.model option = 
  ...
  L.log L.Verboser (lazy (Printf.sprintf "The solver returned: %s" (Solver.string_of_status ret))); 
        
  (* Step 4: BREAK if ret = UNKNOWN *)
  if (ret = Solver.UNKNOWN) then (
    let str = Z3.SMT.benchmark_to_smtstring ctx "Oops" "AUFNIRA" "Unknown" "" [] (Z3.Boolean.mk_and ctx (Solver.get_assertions masterSolver)) in 
...
let check_sat (fs : Formula.Set.t) (gamma : TypEnv.t) : bool = 
  ...
  let ret = 
    if cached then (
      ...
      L.log L.Verboser (lazy ("Finished checking SAT."));
...
let lift_z3_model (model : Model.model) (gamma : TypEnv.t) (vars : SS.t) : SSubst.t = 
