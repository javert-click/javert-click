...
type t =
  ...
  | Unfold           of JSAsrt.t * ((string * ((string * JSExpr.t) list)) option) (** Single Unfold *)
...
let rec js2jsil
  ...
  match logic_cmd with
    | Fold (Pred (s, les), fold_info) -> [ LCmd.SL (Fold (s, List.map fe les, translate_folding_info fold_info)) ]

    | Flash (Pred (s, les)) ->
      let p_name, les' = s, List.map fe les in
      [ LCmd.SL (Unfold (p_name, les', None, false)); LCmd.SL (Fold (p_name, les', None)) ]

    | Unfold ((Pred (s, les)), unfold_info) ->
      [ LCmd.SL (Unfold (s, List.map fe les, (translate_folding_info unfold_info), false)) ]

    | GUnfold name -> [ LCmd.SL (GUnfold name) ]

    | Assert(assertion, binders) -> 
      let a' = JSAsrt.js2jsil_tactic cc_tbl vis_tbl fun_tbl fid scope_var assertion in 
      [ LCmd.SL (SepAssert (a', binders)) ] 

    | Macro (s, les) -> [ LCmd.Macro (s, List.map fe les) ]

    | If (le, lcthen, lcelse) -> [ LCmd.If (fe le, List.concat (List.map f lcthen), List.concat (List.map f lcelse)) ]

    | ApplyLemma (lname, lparams) -> [ LCmd.SL (ApplyLem (lname, List.map fe lparams, []))]
...
let str_of_folding_info (unfold_info : (string * ((string * JSExpr.t) list)) option) : string  =
  match unfold_info with
  | None -> ""
  | Some (id, unfold_info_list) ->
      let unfold_info_list =
          List.map (fun (v, le) -> "(" ^ v ^ " := " ^ (JSExpr.str le) ^ ")") unfold_info_list in
