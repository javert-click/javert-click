...
let set_fv_pair (heap : t) (loc : string) (field : Expr.t) (value : Expr.t) : unit =
  L.log L.Verboser (lazy (Printf.sprintf "Setting fv pair: [%s, %s] -> %s" loc (Expr.str field) (Expr.str value)));
  heap.cdmn := Var.Set.remove loc !(heap.cdmn); heap.sdmn := Var.Set.remove loc !(heap.sdmn);
  let is_conc = is_c field && is_c value in 
  let add, sadd, rem, dom = if (is_conc) then heap.cfvl, heap.cdmn, heap.sfvl, heap.cdom else heap.sfvl, heap.sdmn, heap.cfvl, heap.sdom in 
    ...
    let fvrem = SFVL.remove field (Option.default SFVL.empty (Hashtbl.find_opt rem loc)) in 
      ...
      let cdom = Hashtbl.find_opt heap.cdom loc in 
      let sdom = Hashtbl.find_opt heap.sdom loc in 
      (match cdom, sdom with 
      | None, None 
      | None, Some None 
      | Some None, None 
      | Some None, Some None -> ()
      | Some (Some cdom), Some (Some sdom) -> raise (Failure "Domain in both the concrete and symbolic part of the heap.")
      | Some (Some cdom), _ -> 
          let new_dom = Reduction.reduce_lexpr (Expr.NOp (SetUnion, [ ESet [ field ]; cdom ])) in 
          (match is_conc with 
          | true -> Hashtbl.replace heap.cdom loc (Some new_dom)
          | false -> Hashtbl.remove heap.cdom loc; Hashtbl.replace heap.sdom loc (Some new_dom))
      | _, Some (Some sdom) ->  
          let new_dom = Reduction.reduce_lexpr (Expr.NOp (SetUnion, [ ESet [ field ]; sdom ])) in 
          Hashtbl.replace heap.sdom loc (Some new_dom)
      );
...
let str ?(which = None) (heap : t) = 
  let print_all = which = None in 
  let which = Option.default SS.empty which in 
  (* Get the metadata also for printing *)
  let locs = SS.elements which in 
  let which = List.fold_left (fun (ac : SS.t) (loc : string) -> 
    let md = get_met heap loc in 
    (match md with 
    | Some (Expr.Lit (Loc loc))
    | Some (ALoc loc) -> SS.add loc ac 
    | _ -> ac)) which locs in 
  (* FIXME: Get also the metadata if not null *)
  let sorted_locs = SS.elements (if print_all then domain heap else CCommon.SS.filter (fun x -> has_loc heap x) which) in 
  ...
  let strings : string list = List.map (fun (loc, ((fv_pairs, domain), metadata)) ->
      ...
      loc ^ " |-> [" ^  str_fv_pairs ^ " | " ^ domain_str ^ "] " ^ " with metadata " ^ meta_str) 
    sorted_locs_with_vals in 
   "\t" ^ String.concat "\n\t" strings
