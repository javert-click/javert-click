...
let rec lift_logic_expr (e : Expr.t) : (t * t) option = 
  ...
  match e with 
    ...
    | BinOp (e1, Equal, Lit (Bool true))  -> f e1
    | BinOp (Lit (Bool true), Equal, e2)  -> f e2
    | BinOp (e1, Equal, Lit (Bool false)) -> f (UnOp (Not, e1))
    | BinOp (Lit (Bool false), Equal, e2) -> f (UnOp (Not, e2))
...
let rec to_expr (a : t) : Expr.t option = 
  ...
  | Or (a1, a2)   -> 
      (match f a1, f a2 with 
      | Some le1, Some le2 -> Some (Expr.BinOp (le1, BinOp.Or, le2))
      | _ -> None)
...
let subst_expr_for_expr (e_to_subst : Expr.t) (e_subst : Expr.t) (a : t) : t = 
  map None None (Some (Expr.subst_expr_for_expr e_to_subst e_subst)) a