...
let rec clocs (le : t) : SS.t =
  let f = clocs in 
  match le with 
  | Lit (Loc l) -> SS.singleton l
  | Lit (LList les) -> 
      List.fold_left (fun ac cls -> SS.union ac cls) SS.empty (List.map f (List.map (fun x -> Lit x) les))
  | UnOp (_, le) -> f le
  | BinOp (e1, _, e2) -> SS.union (f e1) (f e2)
  | NOp (_, les) | EList les | ESet  les -> 
      List.fold_left (fun ac cls -> SS.union ac cls) SS.empty (List.map f les)
  | _ -> SS.empty
...
let subst_expr_for_expr (le_to_subst : t) (le_subst : t) (le : t) = 
  let f_before e = 
    (match e with 
    | e when e = le_to_subst -> le_subst, false
    | _ -> e, true) in 
  map f_before None le