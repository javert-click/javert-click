...
function initEventTarget(Node, ShadowRoot, DocumentFragment, MouseEvent, Element, Text, Window, Event, ErrorEvent){
    ...
    scopeEvents.window           = WindowInfo.getInstance();
...
function updatePropagationPath(target, slotable, relatedTarget, event, touchTargets, activationTarget, targetOverride, isActivationEvent){
    ...
    while(parent != null){
        ...
        if(parent instanceof scopeEvents.WindowInfo.Window || (parent instanceof scopeEvents.Node.Node && shadowIncludingInclusiveAncestor(root(target), parent))){
...
function execCallBack(callback, opName, event, currentTarget){
    ...
    console.log('I found a callback');
...
ShadowRoot        = { ShadowRoot: ShadowRoot, ShadowRootMode: ShadowRootMode};
...
initEventTarget(Node, ShadowRoot, DocumentFragment, MouseEvent, Element, Text, Window, Event, ErrorEvent);
